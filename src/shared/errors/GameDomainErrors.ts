...
export class GameError extends Error {
  ...
  static fromJSON(json: GameErrorJSON): GameError {
    return new GameError(json.code as GameErrorCode, json.message, json.context, json.isFatal);
  }
}
...
export class NotYourTurnError extends GameError {
  constructor(expectedPlayer: number, actualPlayer: number, context: Record<string, unknown> = {}) {
    super(
...
export class GameNotFoundError extends GameError {
  constructor(gameId: string, context: Record<string, unknown> = {}) {
    super(GameErrorCode.GAME_NOT_FOUND, `Game not found: ${gameId}`, { gameId, ...context }, false);
...
export class GameNotActiveError extends GameError {
  constructor(gameId: string, status: string, context: Record<string, unknown> = {}) {
    super(
...
export class PlayerUnauthorizedError extends GameError {
  constructor(playerId: string, action: string, context: Record<string, unknown> = {}) {
    super(
 ...
 */
export function wrapError(error: unknown, context: Record<string, unknown> = {}): GameError {
  if (isGameError(error)) {
    return error;
  }
