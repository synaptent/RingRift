...
export interface UseSandboxAILoopReturn {
  ...
  runSandboxAiTurnLoop: (engineOverride?: ClientSandboxEngine) => Promise<void>;
}
...
export function useSandboxAILoop({
  ...
  const runSandboxAiTurnLoop = useCallback(
    async (engineOverride?: ClientSandboxEngine) => {
      const engine = engineOverride ?? sandboxEngine;
      if (!engine) return;
      ...
      try {
        ...
        if (finalState.gameStatus === 'active' && next && next.type === 'ai') {
          ...
          window.setTimeout(() => {
            void runSandboxAiTurnLoop(engine);
          }, 200);
    ...
    [sandboxEngine, setSelected, setValidTargets, setSandboxLastProgressAt, setSandboxStallWarning]
  );

  const maybeRunSandboxAiIfNeeded = useCallback(
    (engineOverride?: ClientSandboxEngine) => {
      const engine = engineOverride ?? sandboxEngine;
      if (!engine) return;
