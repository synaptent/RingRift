 ...
 */
export function createRematchExpiredState(gameId: string, nowMs?: number): RematchExpiredState {
  return {
    kind: 'expired',
    gameId,
...
export function requestRematch(
  ...
  if (state.kind !== 'idle') {
    console.warn(`[RematchStateMachine] requestRematch called from invalid state: ${state.kind}`);
  }
 ...
 */
export function isAwaitingLocalResponse(state: RematchState): state is RematchPendingResponseState {
