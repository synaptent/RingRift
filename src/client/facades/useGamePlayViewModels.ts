...
export function useGamePlayViewModels(
  ...
  const decisionHighlights = useMemo(() => {
    ...
    return deriveBoardDecisionHighlights(
      facade.gameState,
      facade.decisionState.pendingChoice
    );
  ...
  const hudViewModel = useMemo<HUDViewModel | null>(() => {
    ...
    const baseHudVM = toHUDViewModel(facade.gameState, {
      connectionStatus: facade.connectionStatus === 'local-only' ? 'connected' : facade.connectionStatus,
    ...
    if (
      baseHudVM &&
      !baseHudVM.decisionPhase &&
      hasOptionalCapture
    ) {
      const actingPlayer = facade.gameState.players.find(
        (p) => p.playerNumber === facade.gameState!.currentPlayer
      );
      const actingPlayerName = actingPlayer?.username || `Player ${actingPlayer?.playerNumber ?? facade.gameState.currentPlayer}`;
  ...
  const victoryViewModel = useMemo<VictoryViewModel | null>(() => {
    ...
    return toVictoryViewModel(
      facade.victoryState,
      facade.gameState.players,
      facade.gameState,
      {
        currentUserId: facade.currentUserId,
