...
export function useCellInteractions(
    ...
    (position: Position, board: BoardState) => {
      ...
      if (gameState.currentPhase === 'ring_placement') {
        ...
        if (!hasStack) {
          ...
          const preferred =
            placeMovesAtPos.find((m) => (m.placementCount ?? 1) === 1) ||
            placeMovesAtPos[0];
      ...
      if (!selected) {
        ...
        if (hasStack && hasMovesFromHere) {
          ...
          onInvalidMove?.(position, analyzeInvalidSelection(gameState, position, {
            hasStack,
            hasMovesFromHere,
            mustMoveFrom,
          }));
  ...
  const handleCellDoubleClick = useCallback(
    (position: Position, board: BoardState) => {
      ...
      if (!canInteract(facade)) {
        const reason = facade.mode === 'spectator'
...
function analyzeInvalidMove(
  ...
  const hasMatchingOrigin = validMoves.some(
    (m) => m.from && positionsEqual(m.from, from)
  );
  ...
  const movesFromOrigin = validMoves.filter(
    (m) => m.from && positionsEqual(m.from, from)
  );
