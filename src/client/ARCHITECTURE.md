...
- **Sidebar composition and analysis tooling:**
  - Compose presentational panels such as `MoveHistory`, `GameEventLog`, `GameHistoryPanel`, `EvaluationPanel`, chat, and board‑controls overlays around the core board/HUD.
  - Keep these panels props‑driven (view models, callbacks) so that replay, evaluation, and history features remain orchestrator‑agnostic.
...
- **Sandbox diagnostics and tooling:**
  ...
  - Gate advanced diagnostics (trace export, stall watchers, replay database) behind explicit UI affordances so the default UX for new players remains simple.
