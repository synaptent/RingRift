...
- **[RR-CANON-R022] Control and cap height.**
  ...
  - **Invariant:** At the end of any turn or phase, `capHeight >= 1` for all existing stacks. A `capHeight` of 0 is only valid as a transient intermediate state during move application (e.g., during capture processing before ownership transfer completes). Once ownership transfers to a new controlling player, their `capHeight` is recomputed and must be >= 1.
...
- **[RR-CANON-R023] Stack mutation operations.**
  - Legal stack mutations are limited to:
    ...
    - Removing an **entire cap** via territory processing (any controlled stack eligible, including standalone rings).
...
- **[RR-CANON-R073] Mandatory phase transitions for currentPhase.**
  - The `currentPhase` field in `GameState` must change at specific points during turn processing:
    ...
    - **territory_processing → forced_elimination:** After all territory regions are processed (or none existed), if P made **no board state changes** during their turn (no ring placed, no stack moved/captured, no territory collapsed) but P still controls at least one stack, `currentPhase` MUST change to `forced_elimination`. **IMPORTANT:** Voluntary skips (`skip_placement`, `skip_territory_processing`, `skip_capture`) do NOT count as board state changes and do NOT prevent forced elimination—they are decisions to forgo actions, not actions themselves. Only moves that actually modify the board (placement, movement, capture, territory collapse) satisfy the "board state change" criterion.
    - **territory_processing → ring_placement (next player):** After all territory regions are processed (or none existed), if P made at least one board state change during their turn (placement, movement, capture, or territory collapse) OR P has no controlled stacks, the turn ends and the next player's `currentPhase` is set to `ring_placement`. Players with `ringsInHand == 0` will emit `no_placement_action` and proceed to `movement`. This ensures all phases are visited with explicit moves per RR-CANON-R075.
...
- **[RR-CANON-R145] Region collapse and elimination.**
  - For a region R being processed for moving player P:
    ...
    4. **Mandatory self-elimination:**
       ...
       - **Cap definition:** The cap consists of all consecutive rings of P's colour counting from the top of the stack. **All controlled stacks outside the region are eligible** cap elimination targets for territory processing:
         ...
         - **(iii) Height-1 standalone ring:** For a single ring of P's colour, the entire stack (the one ring) is eliminated. This is equivalent to removing the stack entirely.
...
- **[RR-CANON-R172] Last-player-standing victory.**
  ...
  - A player P wins by last-player-standing if all of the following hold:
    ...
    - **Third round:** After the second round completes, on the following round P remains the only player who has taken any legal real action.
