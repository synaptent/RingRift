...
def export_replay_dataset_multi(
    ...
    *,
    ...
    try:
        ...
        # April 2026: Board encoding contract validation
        try:
            from app.training.board_encoding_contract import is_valid_channel_count
            # NPZ files here are trusted local training data generated by this project
            _exported = np.load(_tmp_path, mmap_mode="r", allow_pickle=True)
            if "features" in _exported:
                _ch = _exported["features"].shape[1]
                if not is_valid_channel_count(_ch):
                    print(f"[CONTRACT WARNING] Exported NPZ has {_ch} channels — "
                          f"not a known encoding (40/56/64). Training may fail.")
                else:
                    print(f"[CONTRACT] Channel validation passed: {_ch}ch")
        except Exception:
            pass  # Contract check is advisory
