...
def run_game(ai1_factory: AI_FACTORY, ai2_factory: AI_FACTORY, board_type: BoardType, num_players: int):
...
def benchmark_ai(
    ...
    for i in range(games):
        if i % 2 == 0:
            winner = run_game(test_ai_factory, opponent_ai_factory, board_type, num_players)
            if winner == 1:
                wins += 1
        else:
            winner = run_game(opponent_ai_factory, test_ai_factory, board_type, num_players)
            if winner == 2:
                wins += 1
