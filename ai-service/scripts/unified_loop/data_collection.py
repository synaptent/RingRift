...
class StreamingDataCollector:
    ...
    async def _sync_selfplay_jsonl(self, host: HostState):
        ...
        by the standard DB sync. These files are generated by GPU selfplay
        scripts (run_gpu_selfplay.py) and contain high-quality training data.
        """
        ssh_target = f"{host.ssh_user}@{host.ssh_host}"

        # Selfplay patterns to sync
        selfplay_patterns = [
