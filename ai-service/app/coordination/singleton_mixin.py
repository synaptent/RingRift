...
class ThreadSafeSingletonMixin(SingletonMixin[T]):
    ...
    @classmethod
    def _get_or_create_instance(cls, *args: Any, **kwargs: Any) -> T:
...
def singleton(cls: type[T]) -> type[T]:
    ...
    Note: Supports reset_instance() for testing, unlike simpler implementations.
    """
    import functools
    ...
    # Add helper methods
    @classmethod
    def get_instance_wrapper(inner_cls: type[T]) -> T:
        return inner_cls()

    @classmethod
    def has_instance_wrapper(inner_cls: type[T]) -> bool:
        return _instance is not None

    @classmethod
    def reset_instance_wrapper(inner_cls: type[T]) -> None:
        nonlocal _instance, _initialized
        with _lock:
            _instance = None
            _initialized = False

    cls.get_instance = get_instance_wrapper  # type: ignore
    cls.has_instance = has_instance_wrapper  # type: ignore
    cls.reset_instance = reset_instance_wrapper  # type: ignore
