...
This module was extracted from archive and integrated into the main
codebase in December 2025 to enable real-time online learning for EBMO.

Usage:
    from app.ai.ebmo_online_learner import EBMOOnlineLearner, EBMOOnlineConfig

    # Create learner with EBMO network
    learner = EBMOOnlineLearner(network, device='cuda', config=EBMOOnlineConfig())
    ...
    ai = EBMOOnlineAI(player_number=1, config=config, enable_online_learning=True)
    move = ai.select_move(state)
    ai.end_game(winner)  # Triggers learning update
"""
