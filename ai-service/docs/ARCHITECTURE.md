...
| `POST /ai/moves_batch`       | Batch move generation (GPU/tree search)  |
| `POST /ai/evaluate`          | Evaluate position for all players        |
| `POST /ai/evaluate_position` | Evaluate a single position (raw payload) |
...
| `GET /api/replay/games/{game_id}`         | Fetch replay metadata + players |
