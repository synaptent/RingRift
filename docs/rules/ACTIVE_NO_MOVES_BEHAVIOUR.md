...
- **Implementations / tests:**
  ...
  - TS multi-phase orchestrator tests [`tests/scenarios/Orchestrator.Sandbox.multiPhase.test.ts`](../../tests/scenarios/Orchestrator.Sandbox.multiPhase.test.ts:1) and line scenarios [`tests/unit/GameEngine.lines.scenarios.test.ts`](../../tests/unit/GameEngine.lines.scenarios.test.ts:1).
- **RR-CANON mapping:**
...
- **Expected canonical behaviour (RR-CANON):**
  - Per [`RR-CANON-R172`](../../RULES_CANONICAL_SPEC.md:700) and clarifications in [`COMPLETE_RULES.md`]COMPLETE_RULES.md:1376) §13.3, P should win by Last Player Standing once the “exclusive real-action” condition has held for **three consecutive full rounds** (P takes at least one real action on each of their turns; all others have none).
