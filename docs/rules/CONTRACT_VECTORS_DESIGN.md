...
&gt; - Invariant and parity framework in [`INVARIANTS_AND_PARITY_FRAMEWORK.md`](INVARIANTS_AND_PARITY_FRAMEWORK.md:1) and Python strict-invariant guidance in [`STRICT_INVARIANT_SOAKS.md`](../testing/STRICT_INVARIANT_SOAKS.md:1).  
&gt;  
...
- The meta-framework in [`INVARIANTS_AND_PARITY_FRAMEWORK.md`](INVARIANTS_AND_PARITY_FRAMEWORK.md:42), especially `INV-ACTIVE-NO-MOVES`, `INV-TERMINATION`, and `PARITY-TS-PY-CONTRACT-VECTORS`.
...
3. **C – Territory and Line Interactions Near Endgame**
   ...
   - Strongly related to `PARITY-TS-PY-TERRITORY-LINE` and the analysis in [`RULES_TERMINATION_ANALYSIS.md`](../supplementary/RULES_TERMINATION_ANALYSIS.md:19).
...
- `assertions.sInvariantDelta`: `S(next) - S(prev)` where `S` is defined in [`computeProgressSnapshot`](../../src/shared/engine/core.ts:1) and analysed in [`INVARIANTS_AND_PARITY_FRAMEWORK.md`](INVARIANTS_AND_PARITY_FRAMEWORK.md:84).
...
| `forced_elimination.monotone_chain.final.square8` | same                                  | `square8`  | Final forced elimination removes last ring(s) from the trapped stack.                                                                 | After move, `stackCount` for P1-controlled stacks is 0, `totalRingsEliminated` monotone; either victory is triggered (if threshold reached) or P1 is permanently eliminated and not left as `currentPlayer` with `gameStatus = "active"`. |

#### B2 – Turn rotation skipping permanently eliminated player (3-player square8)

**Sequence tag:** `sequence:forced_elimination.rotation.skip_eliminated.square8`

| ID                                                    | Category    | Board type | Initial situation                                                                                                                                                                                    | Expected outcome                                                                                                                                                                                                            |
| ----------------------------------------------------- | ----------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `forced_elimination.rotation.skip_eliminated.square8` | `edge_case` | `square8`  | 3-player game: P1 has a trapped stack and `ringsInHand = 0`; P2 has no stacks and no rings (permanently eliminated); P3 has rings and at least one legal move. Forced elimination is applied for P1. | After the move and any automatic `_end_turn` logic, `currentPlayer = 3`, `gameStatus = "active"`, P2 remains permanently eliminated and is **not** chosen as `currentPlayer`, consistent with `INV-ANM-TURN-MATERIAL-SKIP`. |
...
| ID                                           | Category               | Board type | Initial situation                                                                                                                                                                                                                                                                               | Expected outcome                                                                                                                                                                                                                                                        |
| -------------------------------------------- | ---------------------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `territory_line.single_point_swing.square19` | `territory_processing` | `square19` | Late-game state on `square19` where P1 and P2 are within one point in the combined elimination/territory ladder; a `choose_territory_option` move for P1 both collapses a 1-cell region and forces mandatory self-elimination from a specific stack (legacy alias: `process_territory_region`). | `status = "complete"`, `gameStatus = "completed"`, `victoryWinner` equals the player favoured by the canonical tiebreak ladder, `player1TerritorySpaces` and `player1EliminatedRings` reflect the exact change, `totalRingsEliminated` monotone, `sInvariantDelta ≥ 1`. |
...
- **Family B – Forced elimination and ANM**
  ...
  - `INV-ANM-TURN-MATERIAL-SKIP` for turn rotation over permanently eliminated players.
...
2. **Cross-reference with seed/plateau harnesses**
   - Where plateau or seed-based parity harnesses exist (see [`test_ts_seed_plateau_snapshot_parity.py`](../../ai-service/tests/parity/test_ts_seed_plateau_snapshot_parity.py:1) and [`PARITY_SEED_TRIAGE.md`](PARITY_SEED_TRIAGE.md:1)), link problem seeds back to the closest contract-vector family in this design (e.g. A/B/C/D) to avoid redundant bespoke fixtures.

3. **Maintain SSOT alignment**
   - Treat this design and the P18.5‑1 section in [`PYTHON_PARITY_REQUIREMENTS.md`](PYTHON_PARITY_REQUIREMENTS.md:1) as **derived**, and always prefer:
