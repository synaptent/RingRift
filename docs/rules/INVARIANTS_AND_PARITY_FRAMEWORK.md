...
This document is a navigation aid over the canonical TS engine and orchestrator under [`src/shared/engine`](../../src/shared/engine), the Python rules mirror under [`ai-service/app`](../../ai-service/app/game_engine/__init__.py), and the production hosts (backend `GameEngine`, sandbox `ClientSandboxEngine`, Python self‑play). It does **not** redefine rules semantics; when in doubt, defer to the SSoTs listed above.
...
- [`docs/rules/RULES_SSOT_MAP.md`](RULES_SSOT_MAP.md) already contains a short "Invariants & Soaks Overview"; this framework expands that section into a structured catalogue with IDs and mappings.
- [`docs/testing/STRICT_INVARIANT_SOAKS.md`](../testing/STRICT_INVARIANT_SOAKS.md) is the operational recipe for strict no‑move and S‑invariant soaks; this doc treats those soaks as **implementations** of the invariant IDs below.
- [`docs/rules/PYTHON_PARITY_REQUIREMENTS.md`](PYTHON_PARITY_REQUIREMENTS.md) defines function/type-level parity between TS and Python; this doc focuses on **behavioural** and **host-level** parity.
...
- Backend and sandbox hosts via [`BoardManager`](../../src/server/game/BoardManager.ts) and [`ClientSandboxEngine`](../../src/client/sandbox/ClientSandboxEngine.ts).
- Python engine via [`BoardState`](../../ai-service/app/models/core.py) and [`BoardManager`](../../ai-service/app/board_manager.py).
...
| Channel          | Mechanisms                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **TS tests**     | [`BoardManager.assertBoardInvariants()`](../../src/server/game/BoardManager.ts), [`ClientSandboxEngine.assertBoardInvariants()`](../../src/client/sandbox/ClientSandboxEngine.ts), shared-engine tests such as [`territoryDecisionHelpers.shared.test.ts`](../../tests/unit/territoryDecisionHelpers.shared.test.ts), [`BoardManager.territoryDisconnection.test.ts`](../../tests/unit/BoardManager.territoryDisconnection.test.ts). |
| **TS soaks**     | [`scripts/run-orchestrator-soak.ts`](../../scripts/run-orchestrator-soak.ts) via `checkStructuralInvariants` and violation IDs `NEGATIVE_STACK_HEIGHT`, `STACK_HEIGHT_MISMATCH`, `INVALID_CAP_HEIGHT`, `NEGATIVE_ELIMINATED_RINGS`.                                                                                                                                                                                                  |
| **Python tests** | Structural expectations embedded in [`test_mutable_state.py`](../../ai-service/tests/test_mutable_state.py), [`test_rules_evaluate_move.py`](../../ai-service/tests/test_rules_evaluate_move.py), and parity tests that compare stacks/markers between TS and Python.                                                                                                                                                                |
| **Ops / alerts** | Any structural violation detected by the TS soak harness contributes to `ringrift_orchestrator_invariant_violations_total` and can trigger `OrchestratorInvariantViolationsStaging` / `OrchestratorInvariantViolationsProduction` in [`alerts.yml`](../../monitoring/prometheus/alerts.yml).                                                                                                                                         |
...
- Let `S = M + C + E` as in [`RR-CANON-R191`](../../RULES_CANONICAL_SPEC.md):
...
- TS shared engine (`computeProgressSnapshot` in [`core.ts`](../../src/shared/engine/core.ts)).
- Backend host via [`GameEngine.appendHistoryEntry`](../../src/server/game/GameEngine.ts).
- Sandbox host via [`ClientSandboxEngine.appendHistoryEntry`](../../src/client/sandbox/ClientSandboxEngine.ts).
...
| Channel                  | Mechanisms                                                                                                                                                                                                                                                                                                                                                      |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **TS tests**             | Shared-engine S‑invariant tests such as [`ProgressSnapshot.core.test.ts`](../../tests/unit/ProgressSnapshot.core.test.ts), [`SharedMutators.invariants.test.ts`](../../tests/unit/SharedMutators.invariants.test.ts), scenario tests around late‑game progress.                                                                                                 |
| **TS soaks**             | [`run-orchestrator-soak.ts`](../../scripts/run-orchestrator-soak.ts) checks `S` and `totalRingsEliminated` on every turn, emitting `S_INVARIANT_DECREASED` and `TOTAL_RINGS_ELIMINATED_DECREASED` violations. Regression seeds may be promoted into [`OrchestratorSInvariant.regression.test.ts`](../../tests/unit/OrchestratorSInvariant.regression.test.ts).  |
...
- **Global legal actions** for P in `state` (RR-CANON-R200) are the set
  - `G(state, P) = Placements(state, P) ∪ PhaseMoves(state, P) ∪ ForcedElim(state, P)`, where:
    ...
    - `PhaseMoves(state, P)` is the set of phase-local interactive moves currently available to P in `state.currentPhase`:
      ...
      - in `movement`: non-capture moves, overtaking captures, and (when recovery-eligible) `recovery_slide` / `skip_recovery`;
      - in `capture` / `chain_capture`: overtaking capture segments/chains;
    ...
    - `ForcedElim(state, P)` is `{ forced_elimination }` exactly when the precondition of [`RR-CANON-R100`](../../RULES_CANONICAL_SPEC.md) holds in `state` for P (P controls at least one stack and has **no** legal placement, non-capture movement, or overtaking capture). Otherwise it is the empty set.
...
- For every _reachable_ state `state` produced by the TS or Python engines, at every **invariant checkpoint** (post shared-engine `processTurn` / Python `GameEngine.apply_move`, after any automatic phase-exit / forced-elimination chain, and at soak snapshot boundaries):
  - If `state.gameStatus == ACTIVE` and `state.currentPlayer = P`, then:
    - P is **not permanently eliminated** (RR‑CANON‑R201); and
    - If P has turn-material, `G(state, P)` is **non-empty** (i.e. `ANM(state, P)` is false).

  - When P has no turn-material, the invariant does **not** require `G(state, P)` to be non-empty; such turns are traversed via explicit bookkeeping moves (`no_*_action`) and/or recovery decisions when recovery-eligible.

- The **only** allowed shape in which _all_ players have no global legal actions is the global stalemate of [`RR-CANON-R173`](../../RULES_CANONICAL_SPEC.md), which must be resolved immediately into a terminal state. Engines should not surface such a state to hosts or soaks as `gameStatus == ACTIVE`.

- In particular, the following ANM scenario families from [`ACTIVE_NO_MOVES_BEHAVIOUR.md`](ACTIVE_NO_MOVES_BEHAVIOUR.md) must never appear as long-lived ACTIVE states:
  ...
  - **ANM-SCEN-03 – Permanently eliminated current player:** movement phases where `currentPlayer` is permanently eliminated but remains active.
  ...
  - **ANM-SCEN-06 – Global stalemate:** global no-moves shapes that are not immediately converted to terminal stalemate under [`RR-CANON-R173`](../../RULES_CANONICAL_SPEC.md).

**Scope**

- TS shared engine and turn logic ([`turnLogic.advanceTurnAndPhase`](../../src/shared/engine/turnLogic.ts), orchestrator).
...
- Encodes the "no dead player" semantics of [`RR-CANON-R072`](../../RULES_CANONICAL_SPEC.md), the ANM definition and avoidance in the R2xx cluster ([`RR-CANON-R200`–`RR-CANON-R204`](../../RULES_CANONICAL_SPEC.md)), and global-stalemate handling in [`RR-CANON-R173`](../../RULES_CANONICAL_SPEC.md).
- Strong enough to catch the historical bugs catalogued in [`ACTIVE_NO_MOVES_BEHAVIOUR.md`](ACTIVE_NO_MOVES_BEHAVIOUR.md) and the Python invariant suites:
  - Movement phases with forced elimination available but not considered (`ANM-SCEN-01`; [`test_active_no_moves_movement_forced_elimination_regression.py`](../../ai-service/tests/invariants/test_active_no_moves_movement_forced_elimination_regression.py)).
  - Movement phases with placements-only global actions ignored by the invariant (`ANM-SCEN-02`; [`test_active_no_moves_movement_placements_only_regression.py`](../../ai-service/tests/invariants/test_active_no_moves_movement_placements_only_regression.py)).
  - Permanently eliminated players left as `currentPlayer` (`ANM-SCEN-03`; [`test_active_no_moves_movement_fully_eliminated_regression.py`](../../ai-service/tests/invariants/test_active_no_moves_movement_fully_eliminated_regression.py)).
  - Line and Territory phases that strand the current player with no decisions (`ANM-SCEN-04` / `ANM-SCEN-05`; [`test_active_no_moves_territory_processing_regression.py`](../../ai-service/tests/invariants/test_active_no_moves_territory_processing_regression.py), [`test_active_no_moves_line_processing_regression.py`](../../ai-service/tests/parity/test_active_no_moves_line_processing_regression.py)).
...
| Channel               | Mechanisms                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **TS soaks**          | [`run-orchestrator-soak.ts`](../../scripts/run-orchestrator-soak.ts) enforces an ACTIVE‑no‑moves invariant after shared-engine move enumeration, emitting `ACTIVE_NO_MOVES` / `ACTIVE_NO_CANDIDATE_MOVES` violations when `gameStatus == 'active'` but no legal or candidate moves exist for `currentPlayer`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **TS tests**          | Decision/turn and multi‑phase tests such as [`GameEngine.lines.scenarios.test.ts`](../../tests/unit/GameEngine.lines.scenarios.test.ts), [`BoardManager.territoryDisconnection.test.ts`](../../tests/unit/BoardManager.territoryDisconnection.test.ts), and orchestrator multi‑phase suites under the `orchestrator-parity` job in [`ORCHESTRATOR_ROLLOUT_PLAN.md`](../architecture/ORCHESTRATOR_ROLLOUT_PLAN.md).                                                                                                                                                                                                                                                                                                                                                                                                   |
| **Python invariants** | Strict invariant `_assert_active_player_has_legal_action` in [`GameEngine`](../../ai-service/app/game_engine/__init__.py), with ANM regression tests [`test_active_no_moves_movement_forced_elimination_regression.py`](../../ai-service/tests/invariants/test_active_no_moves_movement_forced_elimination_regression.py), [`test_active_no_moves_movement_placements_only_regression.py`](../../ai-service/tests/invariants/test_active_no_moves_movement_placements_only_regression.py), [`test_active_no_moves_movement_fully_eliminated_regression.py`](../../ai-service/tests/invariants/test_active_no_moves_movement_fully_eliminated_regression.py), [`test_active_no_moves_territory_processing_regression.py`](../../ai-service/tests/invariants/test_active_no_moves_territory_processing_regression.py). |
| **Python soaks**      | [`run_self_play_soak.py`](../../ai-service/scripts/run_self_play_soak.py) under `RINGRIFT_STRICT_NO_MOVE_INVARIANT=1` treats `no_legal_moves_for_current_player` as an anomaly and can `--fail-on-anomaly`. Snapshots under `ai-service/logs/invariant_failures/` are promoted into the invariant regression tests above.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
- This area is primarily captured by invariants:
  ...
  - `INV-ANM-TURN-MATERIAL-SKIP` – turn rotation never leaves a permanently eliminated player as `currentPlayer` in an ACTIVE state.
  - `INV-TERMINATION` – finite termination and avoidance of progress stalls, using the S- and T-metrics from [`RR-CANON-R191`](../../RULES_CANONICAL_SPEC.md) / [`RR-CANON-R207`](../../RULES_CANONICAL_SPEC.md).
...
- **INV-PHASE-CONSISTENCY (decision phases and automatic exits)**
  - Phase progression must be deterministic and consistent with [`RR-CANON-R070`–`RR-CANON-R072`](../../RULES_CANONICAL_SPEC.md) and the ANM cluster [`RR-CANON-R204`](../../RULES_CANONICAL_SPEC.md):
    - If `currentPhase == line_processing` and the moving player P has **no** legal line decisions (`process_line` / `choose_line_option`), the engine must **immediately** advance out of `line_processing`:
      ...
      - directly to victory evaluation and turn rotation per [`RR-CANON-R170`–`RR-CANON-R173`](../../RULES_CANONICAL_SPEC.md).
    ...
    - If `currentPhase == territory_processing` and P has **no** legal Territory decisions (`choose_territory_option`, `eliminate_rings_from_stack`), the engine must:
      - either apply forced elimination for P if required by [`RR-CANON-R072` / `RR-CANON-R100`](../../RULES_CANONICAL_SPEC.md); or
      - end the turn, rotate `currentPlayer` to the next player in seat order who is not permanently eliminated (RR‑CANON‑R201), and evaluate victory per [`RR-CANON-R170`–`RR-CANON-R173`](../../RULES_CANONICAL_SPEC.md).
    ...
    - For `ring_placement`, `movement`, `capture`, and `chain_capture`, phase rules [`RR-CANON-R070`–`RR-CANON-R072`](../../RULES_CANONICAL_SPEC.md) together with `INV-ACTIVE-NO-MOVES` already ensure that whenever P has turn-material, at least one global legal action (possibly forced elimination) exists.

- **INV-ANM-TURN-MATERIAL-SKIP (turn rotation skips permanently eliminated players)**
  - A player P is **permanently eliminated** when P has **no rings anywhere**: no controlled stacks, no rings in hand, and no buried rings (RR‑CANON‑R201).

  - In any state with `gameStatus == ACTIVE`:
    - `currentPlayer` must never be permanently eliminated.
    - Turn rotation (`endTurn` / `_end_turn`) must skip permanently eliminated players and choose the next player in seat order who is not permanently eliminated. This includes players who have no turn-material but still have buried rings (temporarily eliminated) and/or are recovery-eligible.

  - This invariant rules out ANM-SCEN-03 and the permanent-elimination cases in ANM-SCEN-08, where a permanently eliminated player improperly retains the turn.
...
- Documented in [`docs/incidents/INCIDENT_TERRITORY_MUTATOR_DIVERGENCE.md`](../incidents/INCIDENT_TERRITORY_MUTATOR_DIVERGENCE.md), the ANM scenario catalogue [`ACTIVE_NO_MOVES_BEHAVIOUR.md`](ACTIVE_NO_MOVES_BEHAVIOUR.md), and the Python active-no-moves regression tests.

**Enforcement mapping**

| Channel          | Mechanisms                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **TS tests**     | Turn/phase and Territory tests under [`territoryDecisionHelpers.shared.test.ts`](../../tests/unit/territoryDecisionHelpers.shared.test.ts), [`BoardManager.territoryDisconnection.test.ts`](../../tests/unit/BoardManager.territoryDisconnection.test.ts), and orchestrator multi‑phase scenario suites listed under the `orchestrator-parity` CI job in [`ORCHESTRATOR_ROLLOUT_PLAN.md`](../architecture/ORCHESTRATOR_ROLLOUT_PLAN.md). |
| **Python tests** | The ANM invariant suites (see `INV-ACTIVE-NO-MOVES`) plus [`test_territory_forced_elimination_divergence.py`](../incidents/INCIDENT_TERRITORY_MUTATOR_DIVERGENCE.md) validate that `_update_phase` and `_end_turn` mirror TS phase semantics and correctly skip permanently eliminated players.                                                                                                                                          |
...
- Rules-level termination is justified by the S‑invariant `S = M + C + E` in [`RR-CANON-R191`](../../RULES_CANONICAL_SPEC.md) together with ANM and forced-elimination semantics in [`RR-CANON-R207`](../../RULES_CANONICAL_SPEC.md). For invariant checks and soaks we refine this into two concrete, computable progress measures:
...
- **Progress under forced-elimination-only sequences**
  - For any action classified as **host-level forced elimination** under [`RR-CANON-R205`](../../RULES_CANONICAL_SPEC.md) (RR-CANON-R100 precondition holds and the engine applies the `forced_elimination` action):
...
- For any host‑enumerated move `m` in a given state `s`:
  - The shared orchestrator's [`validateMove()`](../../src/shared/engine/orchestration/turnOrchestrator.ts) must mark `m` as valid.
...
| Channel          | Mechanisms                                                                                                                                                                                                                                                                                         |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **TS soaks**     | [`run-orchestrator-soak.ts`](../../scripts/run-orchestrator-soak.ts) re‑validates every move returned by `GameEngine.getValidMoves` via [`validateMove()`](../../src/shared/engine/orchestration/turnOrchestrator.ts), emitting `ORCHESTRATOR_VALIDATE_MOVE_FAILED` violations when they disagree. |
...
- Game status and victory metadata must be consistent with the canonical victory rules [`RR-CANON-R170`–`R173`](../../RULES_CANONICAL_SPEC.md):
...
| Channel              | Mechanisms                                                                                                                                                                                                                                                                                                                                        |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **TS tests**         | Victory scenario suites such as [`GameEngine.victory.scenarios.test.ts`](../../tests/unit/GameEngine.victory.scenarios.test.ts) plus shared‐engine victory tests.                                                                                                                                                                                 |
| **Python tests**     | Victory parity and stalemate tests under [`ai-service/tests/test_engine_correctness.py`](../../ai-service/tests/test_engine_correctness.py) and parity fixtures.                                                                                                                                                                                  |
| **Contract vectors** | Victory‑focused vectors using the `victory` category in [`tests/fixtures/contract-vectors/v2`](../../tests/fixtures/contract-vectors/v2/README.md), enforced by [`contractVectorRunner.test.ts`](../../tests/contracts/contractVectorRunner.test.ts) and [`test_contract_vectors.py`](../../ai-service/tests/contracts/test_contract_vectors.py). |
| **Ops / alerts**     | `RulesParityGameStatusMismatch` alert in [`alerts.yml`](../../monitoring/prometheus/alerts.yml) fires when TS and Python disagree on game status or winners; this is treated as a critical breach of `INV-GAME-STATUS`.                                                                                                                           |
...
- For all v2 contract vectors under [`tests/fixtures/contract-vectors/v2`](../../tests/fixtures/contract-vectors/v2):
  - TS [`processTurn()`](../../src/shared/engine/orchestration/turnOrchestrator.ts) and Python [`GameEngine.apply_move()`](../../ai-service/app/game_engine/__init__.py) must produce:
...
| Channel          | Mechanisms                                                                                                                                                                                                                                                                                                             |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **TS tests**     | [`contractVectorRunner.test.ts`](../../tests/contracts/contractVectorRunner.test.ts) loads v2 bundles and runs each vector through [`processTurn()`](../../src/shared/engine/orchestration/turnOrchestrator.ts).                                                                                                       |
| **Python tests** | [`test_contract_vectors.py`](../../ai-service/tests/contracts/test_contract_vectors.py) runs the same vectors against [`GameEngine.apply_move()`](../../ai-service/app/game_engine/__init__.py).                                                                                                                       |
| **CI**           | `orchestrator-parity` job in [`.github/workflows/ci.yml`](../../.github/workflows/ci.yml) runs `npm run test:orchestrator-parity:ts` followed by `./scripts/run-python-contract-tests.sh --verbose`, enforcing `SLO-CI-ORCH-PARITY` in [`ORCHESTRATOR_ROLLOUT_PLAN.md`](../architecture/ORCHESTRATOR_ROLLOUT_PLAN.md). |
...
| Channel          | Mechanisms                                                                                                                                                                                                                                                                                                                                                                                |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Python tests** | [`test_line_and_territory_scenario_parity.py`](../../ai-service/tests/parity/test_line_and_territory_scenario_parity.py) constructs synthetic line + region setups for square8/square19/hexagonal and asserts behaviour consistent with TS semantics (from RulesMatrix/Q7/Q20/Q22).                                                                                                       |
| **TS tests**     | Scenario suites such as [`GameEngine.lines.scenarios.test.ts`](../../tests/unit/GameEngine.lines.scenarios.test.ts), [`RulesMatrix.Territory.MiniRegion.test.ts`](../../tests/scenarios/RulesMatrix.Territory.MiniRegion.test.ts), and FAQ/Q15–Q23 tests encode the TS reference behaviour.                                                                                               |
...
| Channel          | Mechanisms                                                                                                                                                                                                                                                            |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Python tests** | Seed/plateau parity suites under [`ai-service/tests/parity`](../../ai-service/tests/parity/test_ts_seed_plateau_snapshot_parity.py), [`test_rules_parity_fixtures.py`](../../ai-service/tests/parity/test_rules_parity_fixtures.py), and related plateau diagnostics. |
| **TS tests**     | Legacy trace replay in [`TraceFixtures.sharedEngineParity.test.ts`](../../tests/unit/TraceFixtures.sharedEngineParity.test.ts) and archived backend‑vs‑sandbox parity in [`archive/tests/unit`](../../archive/tests/unit/Backend_vs_Sandbox.aiParallelDebug.test.ts). |
...
- For identical starting states and move/decision sequences:
  - Backend `GameEngine` (via [`TurnEngineAdapter`](../../src/server/game/turn/TurnEngineAdapter.ts)) and sandbox [`ClientSandboxEngine`](../../src/client/sandbox/ClientSandboxEngine.ts) (via [`SandboxOrchestratorAdapter`](../../src/client/sandbox/SandboxOrchestratorAdapter.ts)) must:
...
| Channel      | Mechanisms                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **TS tests** | Backend vs sandbox suites such as [`ClientSandboxEngine.lines.test.ts`](../../tests/unit/sandboxLines.test.ts), [`ClientSandboxEngine.territoryDisconnection.hex.test.ts`](../../tests/unit/ClientSandboxEngine.territoryDisconnection.hex.test.ts), and orchestrator multi‑phase parity tests (`Orchestrator.Backend.multiPhase`, `Orchestrator.Sandbox.multiPhase`) run under `npm run test:orchestrator-parity:ts` as documented in [`ORCHESTRATOR_ROLLOUT_PLAN.md`](../architecture/ORCHESTRATOR_ROLLOUT_PLAN.md). |
| **CI**       | `orchestrator-parity` job enforces that both hosts, when using orchestrator adapters, stay aligned with the shared engine.                                                                                                                                                                                                                                                                                                                                                                                             |
| **Alerts**   | Host-parity issues discovered via parity harnesses contribute to `RulesParity*` metrics. Runtime python‑authoritative diagnostics surface them via `ringrift_rules_parity_mismatches_total{suite="runtime_python_mode",...}` when enabled.                                                                                                                                                                                                                                                                             |
...
- The interpretation of "legal actions exist for the active player" must match between TS and Python:
  - Placements, movements/captures, recovery actions (when recovery-eligible), and forced eliminations count as legal actions.
  - Permanently eliminated players and territory/line decision-phase exits must rotate turns consistently.
...
| **Python**      | Strict no‑move invariant and regression tests in [`ai-service/tests/invariants`](../../ai-service/tests/invariants/test_active_no_moves_movement_forced_elimination_regression.py).                                                                        |
...
| `INV-S-MONOTONIC`, `INV-ACTIVE-NO-MOVES`, `INV-TERMINATION`                                                                         | `python-ai-healthcheck` (CI job), `AI Self-Play Healthcheck (Nightly)` workflow | The CI job and nightly workflow defined in [`.github/workflows/ci.yml`](../../.github/workflows/ci.yml) and [`.github/workflows/ai-healthcheck-nightly.yml`](../../.github/workflows/ai-healthcheck-nightly.yml) run `run_self_play_soak.py --profile ai-healthcheck` under strict invariant, surfacing Python-side invariant regressions across square8/square19/hexagonal as a **P1/P2 health signal** that complements the orchestrator-centric SLO gates. |
| `PARITY-BACKEND-SANDBOX-TS`                                                                                                         | `orchestrator-parity`                                                           | Backend vs sandbox orchestrator multi‑phase suites. Explicitly tied to the **Capture/Territory Host Parity** weakest aspect in [`ORCHESTRATOR_ROLLOUT_PLAN.md`](../architecture/ORCHESTRATOR_ROLLOUT_PLAN.md).                                                                                                                                                                                                                                                |
...
| `PARITY-BACKEND-SANDBOX-TS` (legacy vs orchestrator, historical)                                           | None (legacy path removed)                                                                                                                | Shadow mode was removed; there is no runtime alert for legacy vs orchestrator. Use `RulesParity*` alerts and `ringrift_rules_parity_mismatches_total{suite="runtime_python_mode",...}` when running python‑authoritative diagnostics.                                                                                                                                                                                                                                                                                                  |
...
- **Last-Player-Standing (RR-CANON-R172) implementation strength**
  - The canonical Last-Player-Standing semantics in [`RR-CANON-R172`](../../RULES_CANONICAL_SPEC.md) rely on the distinction between real actions and forced eliminations. Current engines approximate this behaviour but do not yet implement the full "exclusive real-action round" condition (see ANM-SCEN-07 and [`RULES_CONSISTENCY_EDGE_CASES.md`](../supplementary/RULES_CONSISTENCY_EDGE_CASES.md)).
...
- **B2‑3 – Sandbox soak harness (orchestrator-backed)** (`INV-ACTIVE-NO-MOVES`, `PARITY-BACKEND-SANDBOX-TS`)
  - Reintroduce a sandbox‑based variant of the orchestrator soak harness once `ClientSandboxEngine` + [`SandboxOrchestratorAdapter`](../../src/client/sandbox/SandboxOrchestratorAdapter.ts) are fully CI‑clean.
...
- **B3‑1 – Python S‑invariant check in soaks** (`INV-S-MONOTONIC`, `INV-ELIMINATION-MONOTONIC`)
  - Extend [`run_self_play_soak.py`](../../ai-service/scripts/run_self_play_soak.py) to compute `S` and `total_rings_eliminated` per move and flag any decreases when strict invariant is enabled.
...
- **B4‑1 – Per‑invariant metrics with IDs** (`INV-STATE-STRUCTURAL`, `INV-S-MONOTONIC`, `INV-ACTIVE-NO-MOVES`, `INV-ORCH-VALIDATION`, `INV-TERMINATION`)
  ...
  - Update [`monitoring/prometheus/alerts.yml`](../../monitoring/prometheus/alerts.yml) and [`docs/operations/ALERTING_THRESHOLDS.md`](../operations/ALERTING_THRESHOLDS.md) to document and use these labels.
...
- **B5‑1 – Link existing docs to this framework** (all IDs)
  - Update [`docs/testing/STRICT_INVARIANT_SOAKS.md`](../testing/STRICT_INVARIANT_SOAKS.md), [`docs/rules/RULES_SSOT_MAP.md`](RULES_SSOT_MAP.md), and [`docs/rules/PYTHON_PARITY_REQUIREMENTS.md`](PYTHON_PARITY_REQUIREMENTS.md) to reference this file by name and reuse invariant/parity IDs where they discuss the same concepts.
...
- **B6‑1 – Nightly AI self‑play invariant dashboard** (`INV-ACTIVE-NO-MOVES`, `INV-TERMINATION`, `INV-S-MONOTONIC`)
  - Define a nightly job that runs [`run_self_play_soak.py`](../../ai-service/scripts/run_self_play_soak.py) under strict invariant for a representative mix of board types and players.
...
| ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `INV-ACTIVE-NO-MOVES`                           | Active player in an ACTIVE state always has at least one global legal action (placements, phase-local moves, or forced elimination); ANM states are resolved immediately or only arise as terminal stalemate.                                       | [`RR-CANON-R200`–`RR-CANON-R204`](../../RULES_CANONICAL_SPEC.md), [`RR-CANON-R072`](../../RULES_CANONICAL_SPEC.md), [`RR-CANON-R100`](../../RULES_CANONICAL_SPEC.md), [`RR-CANON-R173`](../../RULES_CANONICAL_SPEC.md)                                 | ANM-SCEN-01..06 in [`ACTIVE_NO_MOVES_BEHAVIOUR.md`](ACTIVE_NO_MOVES_BEHAVIOUR.md)  | TS orchestrator soaks (`ACTIVE_NO_MOVES` / `ACTIVE_NO_CANDIDATE_MOVES`), TS turn/phase scenario tests, Python strict no-move invariant and regression tests (`test_active_no_moves_*`), Python self-play soaks with strict invariant enabled.                   |
| `INV-PHASE-CONSISTENCY`                         | Composite phases (`line_processing`, `territory_processing`) auto-exit when no local decisions remain, handing control to either further decision phases, forced elimination, or the next player's turn.                                            | [`RR-CANON-R070`–`RR-CANON-R072`](../../RULES_CANONICAL_SPEC.md), [`RR-CANON-R204`](../../RULES_CANONICAL_SPEC.md), [`RR-CANON-R120`–`RR-CANON-R122`](../../RULES_CANONICAL_SPEC.md), [`RR-CANON-R140`–`RR-CANON-R145`](../../RULES_CANONICAL_SPEC.md) | ANM-SCEN-04, ANM-SCEN-05                                                           | TS line/territory helper tests, orchestrator multi-phase suites, Python territory/phase regression tests (`test_active_no_moves_territory_processing_regression.py`, `test_active_no_moves_line_processing_regression.py`).                                     |
| `INV-ANM-TURN-MATERIAL-SKIP`                    | Turn rotation skips permanently eliminated players: `currentPlayer` in ACTIVE states is never permanently eliminated, and rotation selects the next non-permanently-eliminated seat (including temporarily eliminated / recovery-eligible players). | [`RR-CANON-R201`](../../RULES_CANONICAL_SPEC.md), [`RR-CANON-R070`–`RR-CANON-R072`](../../RULES_CANONICAL_SPEC.md)                                                                                                                                     | ANM-SCEN-03, ANM-SCEN-08                                                           | TS turn-rotation logic tests, Python `_end_turn` invariants and [`test_active_no_moves_movement_fully_eliminated_regression.py`](../../ai-service/tests/invariants/test_active_no_moves_movement_fully_eliminated_regression.py), multi-player self-play soaks. |
| `INV-S-MONOTONIC` / `INV-ELIMINATION-MONOTONIC` | `S = M + C + E` and total eliminated rings are non-decreasing under all legal actions; supports global termination arguments.                                                                                                                       | [`RR-CANON-R191`](../../RULES_CANONICAL_SPEC.md), [`RR-CANON-R207`](../../RULES_CANONICAL_SPEC.md)                                                                                                                                                     | ANM-SCEN-01, ANM-SCEN-06, plateau chains referenced by `PARITY-TS-PY-SEED-PLATEAU` | TS shared-engine progress tests and orchestrator soaks (`S_INVARIANT_DECREASED`, `TOTAL_RINGS_ELIMINATED_DECREASED`), Python parity tests for progress snapshots.                                                                                               |
| `INV-TERMINATION`                               | No infinite play: forced-elimination-only sequences and decision-phase chains must make progress in `C + E`, and long games rarely hit configured turn limits.                                                                                      | [`RR-CANON-R191`](../../RULES_CANONICAL_SPEC.md), [`RR-CANON-R207`](../../RULES_CANONICAL_SPEC.md), [`RR-CANON-R170`–`RR-CANON-R173`](../../RULES_CANONICAL_SPEC.md)                                                                                   | ANM-SCEN-01, ANM-SCEN-04..07; long plateau seeds in `PARITY-TS-PY-SEED-PLATEAU`    | TS orchestrator soaks (`hitMaxTurns`), Python self-play stability and soak tests, AI healthcheck profiles.                                                                                                                                                      |
| `PARITY-TS-PY-ACTIVE-NO-MOVES`                  | TS and Python agree on `ANM(state, P)` and on how ANM states are resolved (forced elimination vs rotation vs stalemate).                                                                                                                            | Same as `INV-ACTIVE-NO-MOVES`, plus [`RR-CANON-R205`–`RR-CANON-R207`](../../RULES_CANONICAL_SPEC.md)                                                                                                                                                   | ANM-SCEN-01..06                                                                    | Python ANM invariants/parity tests vs TS orchestrator behaviour, ANM-focused contract vectors and plateau fixtures.                                                                                                                                             |
| `PARITY-TS-PY-SEED-PLATEAU`                     | Long-running plateau traces have matching hashes, S / T progress, and game status across TS and Python.                                                                                                                                             | [`RR-CANON-R191`](../../RULES_CANONICAL_SPEC.md), [`RR-CANON-R170`–`RR-CANON-R173`](../../RULES_CANONICAL_SPEC.md)                                                                                                                                     | Plateau and late-game ANM shapes; especially ANM-SCEN-01, ANM-SCEN-06, ANM-SCEN-07 | TS trace replay and plateau tests, Python plateau parity suites under [`ai-service/tests/parity`](../../ai-service/tests/parity/test_ts_seed_plateau_snapshot_parity.py).                                                                                       |
| `PARITY-TS-PY-CONTRACT-VECTORS`                 | Canonical contract vectors, including those covering ANM, forced elimination, and termination shapes, produce identical next states and invariant deltas in TS and Python.                                                                          | Full ruleset, especially R2xx and [`RR-CANON-R170`–`RR-CANON-R173`](../../RULES_CANONICAL_SPEC.md)                                                                                                                                                     | Selected ANM- and termination-focused vectors; overlaps ANM-SCEN-01..06            | TS/Python contract vector runners and the `orchestrator-parity` CI job.                                                                                                                                                                                         |
