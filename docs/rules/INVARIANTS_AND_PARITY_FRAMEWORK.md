...
| Channel          | Mechanisms                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **TS tests**     | [`BoardManager.assertBoardInvariants()`](../../src/server/game/BoardManager.ts:37), [`ClientSandboxEngine.assertBoardInvariants()`](../../src/client/sandbox/ClientSandboxEngine.ts:2337), shared-engine tests such as [`territoryDecisionHelpers.shared.test.ts`](../../tests/unit/territoryDecisionHelpers.shared.test.ts:1), [`BoardManager.territoryDisconnection.test.ts`](../../tests/unit/BoardManager.territoryDisconnection.test.ts:1). |
...
- In particular, the following ANM scenario families from [`ACTIVE_NO_MOVES_BEHAVIOUR.md`](ACTIVE_NO_MOVES_BEHAVIOUR.md:35) must never appear as long-lived ACTIVE states:
...
- Strong enough to catch the historical bugs catalogued in [`ACTIVE_NO_MOVES_BEHAVIOUR.md`](ACTIVE_NO_MOVES_BEHAVIOUR.md:35) and the Python invariant suites:
  ...
  - Permanently eliminated players left as `currentPlayer` (`ANM-SCEN-03`; [`test_active_no_moves_movement_fully_eliminated_regression.py`](../../ai-service/tests/invariants/test_active_no_moves_movement_fully_eliminated_regression.py:1)).
...
- **INV-PHASE-CONSISTENCY (decision phases and automatic exits)**
  - Phase progression must be deterministic and consistent with [`RR-CANON-R070`–`RR-CANON-R072`](../../RULES_CANONICAL_SPEC.md:192) and the ANM cluster [`RR-CANON-R204`](../../RULES_CANONICAL_SPEC.md:256):
    ...
    - If `currentPhase == territory_processing` and P has **no** legal Territory decisions (`choose_territory_option`, `eliminate_rings_from_stack`), the engine must:
      ...
      - end the turn, rotate `currentPlayer` to the next player in seat order who is not permanently eliminated (RR‑CANON‑R201), and evaluate victory per [`RR-CANON-R170`–`RR-CANON-R173`](../../RULES_CANONICAL_SPEC.md:523).
...
| Channel          | Mechanisms                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **TS tests**     | Turn/phase and Territory tests under [`territoryDecisionHelpers.shared.test.ts`](../../tests/unit/territoryDecisionHelpers.shared.test.ts:1), [`BoardManager.territoryDisconnection.test.ts`](../../tests/unit/BoardManager.territoryDisconnection.test.ts:1), and orchestrator multi‑phase scenario suites listed under the `orchestrator-parity` CI job in [`ORCHESTRATOR_ROLLOUT_PLAN.md`](../architecture/ORCHESTRATOR_ROLLOUT_PLAN.md:739). |
| **Python tests** | The ANM invariant suites (see `INV-ACTIVE-NO-MOVES`) plus [`test_territory_forced_elimination_divergence.py`](../incidents/INCIDENT_TERRITORY_MUTATOR_DIVERGENCE.md:229) validate that `_update_phase` and `_end_turn` mirror TS phase semantics and correctly skip permanently eliminated players.                                                                                                                                              |
...
- For all v2 contract vectors under [`tests/fixtures/contract-vectors/v2`](../../tests/fixtures/contract-vectors/v2):
  - TS [`processTurn()`](../../src/shared/engine/orchestration/turnOrchestrator.ts:1) and Python [`GameEngine.apply_move()`](../../ai-service/app/game_engine/__init__.py:1) must produce:
...
| **Python tests** | [`test_contract_vectors.py`](../../ai-service/tests/contracts/test_contract_vectors.py:1) runs the same vectors against [`GameEngine.apply_move()`](../../ai-service/app/game_engine/__init__.py:1).                                                                                                                         |
| **CI**           | `orchestrator-parity` job in [`.github/workflows/ci.yml`](../../.github/workflows/ci.yml:1) runs `npm run test:orchestrator-parity:ts` followed by `./scripts/run-python-contract-tests.sh --verbose`, enforcing `SLO-CI-ORCH-PARITY` in [`ORCHESTRATOR_ROLLOUT_PLAN.md`](../architecture/ORCHESTRATOR_ROLLOUT_PLAN.md:438). |
...
| Channel      | Mechanisms                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **TS tests** | Backend vs sandbox suites such as [`ClientSandboxEngine.lines.test.ts`](../../tests/unit/sandboxLines.test.ts:1), [`ClientSandboxEngine.territoryDisconnection.hex.test.ts`](../../tests/unit/ClientSandboxEngine.territoryDisconnection.hex.test.ts:1), and orchestrator multi‑phase parity tests (`Orchestrator.Backend.multiPhase`, `Orchestrator.Sandbox.multiPhase`) run under `npm run test:orchestrator-parity:ts` as documented in [`ORCHESTRATOR_ROLLOUT_PLAN.md`](../architecture/ORCHESTRATOR_ROLLOUT_PLAN.md:739). |
...
| `PARITY-BACKEND-SANDBOX-TS`                                                                                                         | `orchestrator-parity`                                                           | Backend vs sandbox orchestrator multi‑phase suites. Explicitly tied to the **Capture/Territory Host Parity** weakest aspect in [`ORCHESTRATOR_ROLLOUT_PLAN.md`](../architecture/ORCHESTRATOR_ROLLOUT_PLAN.md:636).                                                                                                                                                                                                                                                |
...
- **B4‑1 – Per‑invariant metrics with IDs** (`INV-STATE-STRUCTURAL`, `INV-S-MONOTONIC`, `INV-ACTIVE-NO-MOVES`, `INV-ORCH-VALIDATION`, `INV-TERMINATION`)
  ...
  - Update [`monitoring/prometheus/alerts.yml`](../../monitoring/prometheus/alerts.yml:583) and [`docs/operations/ALERTING_THRESHOLDS.md`](../operations/ALERTING_THRESHOLDS.md:561) to document and use these labels.
