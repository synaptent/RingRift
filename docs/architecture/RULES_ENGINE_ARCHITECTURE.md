...
[`src/shared/engine`](../../src/shared/engine/types.ts) as the primary executable
...
- **Movement & captures**
  - Geometry & non‑capturing reachability: [`movementLogic.ts`](../../src/shared/engine/movementLogic.ts)
  - Overtaking capture enumeration: [`captureLogic.ts`](../../src/shared/engine/captureLogic.ts)
  - Shared helpers for applying movement/capture segments and marker path effects: [`core.ts`](../../src/shared/engine/core.ts), [`movementApplication.ts`](../../src/shared/engine/movementApplication.ts)
  - Aggregates and mutators: [`MovementAggregate.ts`](../../src/shared/engine/aggregates/MovementAggregate.ts), [`CaptureAggregate.ts`](../../src/shared/engine/aggregates/CaptureAggregate.ts), [`MovementMutator.ts`](../../src/shared/engine/mutators/MovementMutator.ts), [`CaptureMutator.ts`](../../src/shared/engine/mutators/CaptureMutator.ts)

- **Lines**
  - Marker‑line geometry: [`lineDetection.ts`](../../src/shared/engine/lineDetection.ts)
  - Canonical decision helpers: [`lineDecisionHelpers.ts`](../../src/shared/engine/lineDecisionHelpers.ts) – enumerate and apply `process_line` and `choose_line_option` decision `Move`s (legacy alias: `choose_line_reward`), and standardise when a line collapse grants a ring‑elimination opportunity (via `pendingLineRewardElimination`) for all hosts.
  - Aggregate: [`LineAggregate.ts`](../../src/shared/engine/aggregates/LineAggregate.ts)

- **Territory (detection, borders, processing)**
  - Region detection: [`territoryDetection.ts`](../../src/shared/engine/territoryDetection.ts)
  - Border‑marker expansion: [`territoryBorders.ts`](../../src/shared/engine/territoryBorders.ts)
  - Region processing pipeline (Q23 / self‑elimination compatible): [`territoryProcessing.ts`](../../src/shared/engine/territoryProcessing.ts)
  - Canonical decision helpers: [`territoryDecisionHelpers.ts`](../../src/shared/engine/territoryDecisionHelpers.ts) – enumerates and applies `choose_territory_option` and `eliminate_rings_from_stack` decisions (legacy alias: `process_territory_region`), enforcing Q23, S‑invariant, and per‑player elimination bookkeeping for backend and sandbox hosts.
  - Aggregate: [`TerritoryAggregate.ts`](../../src/shared/engine/aggregates/TerritoryAggregate.ts)

- **Placement**
  - Placement validation (including no‑dead‑placement wiring): [`PlacementValidator.ts`](../../src/shared/engine/validators/PlacementValidator.ts)
  - Board‑level placement mutators: [`PlacementMutator.ts`](../../src/shared/engine/mutators/PlacementMutator.ts)

- **Victory**
  - Ring‑elimination, Territory‑majority, and stalemate ladder: [`VictoryAggregate.ts`](../../src/shared/engine/aggregates/VictoryAggregate.ts)

- **Turn sequencing & orchestration**
  - Shared phase/turn state machine: [`turnLogic.ts`](../../src/shared/engine/turnLogic.ts)
  - Canonical FSM: [`fsm/TurnStateMachine.ts`](../../src/shared/engine/fsm/TurnStateMachine.ts), [`fsm/FSMAdapter.ts`](../../src/shared/engine/fsm/FSMAdapter.ts)
  - **Canonical Turn Orchestrator:** [`turnOrchestrator.ts`](../../src/shared/engine/orchestration/turnOrchestrator.ts) – single entry point (`processTurn` / `processTurnAsync`) for all turn processing, delegating to domain aggregates ([`PlacementAggregate.ts`](../../src/shared/engine/aggregates/PlacementAggregate.ts), [`MovementAggregate.ts`](../../src/shared/engine/aggregates/MovementAggregate.ts), [`CaptureAggregate.ts`](../../src/shared/engine/aggregates/CaptureAggregate.ts), [`LineAggregate.ts`](../../src/shared/engine/aggregates/LineAggregate.ts), [`TerritoryAggregate.ts`](../../src/shared/engine/aggregates/TerritoryAggregate.ts), [`VictoryAggregate.ts`](../../src/shared/engine/aggregates/VictoryAggregate.ts)) in deterministic order

- **Orchestration (Complete)**
  - Canonical orchestrator: [`src/shared/engine/orchestration/`](../../src/shared/engine/orchestration/README.md)
  - FSM: [`fsm/TurnStateMachine.ts`](../../src/shared/engine/fsm/TurnStateMachine.ts), [`fsm/FSMAdapter.ts`](../../src/shared/engine/fsm/FSMAdapter.ts) (legacy `phaseStateMachine.ts` removed in PASS30-R1)
  - Orchestration types: [`types.ts`](../../src/shared/engine/orchestration/types.ts)
  - Backend adapter: [`TurnEngineAdapter.ts`](../../src/server/game/turn/TurnEngineAdapter.ts)
  - Client adapter: [`SandboxOrchestratorAdapter.ts`](../../src/client/sandbox/SandboxOrchestratorAdapter.ts)
...
- **Contract Testing (NEW)**
  - Contract schemas: [`src/shared/engine/contracts/schemas.ts`](../../src/shared/engine/contracts/schemas.ts)
  - Contract serialization: [`src/shared/engine/contracts/serialization.ts`](../../src/shared/engine/contracts/serialization.ts)
  - Test vector generator: [`src/shared/engine/contracts/testVectorGenerator.ts`](../../src/shared/engine/contracts/testVectorGenerator.ts)
  - Contract test runner (TS): [`tests/contracts/contractVectorRunner.test.ts`](../../tests/contracts/contractVectorRunner.test.ts)
  - Contract test runner (Python): [`ai-service/tests/contracts/test_contract_vectors.py`](../../ai-service/tests/contracts/test_contract_vectors.py)
...
- **Shared engine (pure helpers)** – modules under
  [`src/shared/engine`](../../src/shared/engine/types.ts) implement geometry,
...
- **Canonical Turn Orchestrator (NEW)** – the orchestration layer in
  [`src/shared/engine/orchestration/`](../../src/shared/engine/orchestration/README.md) that:
...
- **Backend host (`TurnEngineAdapter`)** – server‑side adapter in
  [`TurnEngineAdapter.ts`](../../src/server/game/turn/TurnEngineAdapter.ts) that:
...
- **Client sandbox host (`SandboxOrchestratorAdapter`)** – browser‑side adapter in
  [`SandboxOrchestratorAdapter.ts`](../../src/client/sandbox/SandboxOrchestratorAdapter.ts) that:
...
1. Identify the relevant shared module(s):
   - Movement & captures (enumeration, application, chain state, aggregates): [`movementLogic.ts`](../../src/shared/engine/movementLogic.ts),
     [`captureLogic.ts`](../../src/shared/engine/captureLogic.ts),
     [`movementApplication.ts`](../../src/shared/engine/movementApplication.ts),
     [`MovementAggregate.ts`](../../src/shared/engine/aggregates/MovementAggregate.ts),
     [`CaptureAggregate.ts`](../../src/shared/engine/aggregates/CaptureAggregate.ts),
     ...
     [`core.validateCaptureSegmentOnBoard`](../../src/shared/engine/core.ts).
   - Lines (geometry, aggregation, decision phases): [`lineDetection.ts`](../../src/shared/engine/lineDetection.ts),
     [`LineAggregate.ts`](../../src/shared/engine/aggregates/LineAggregate.ts),
     [`lineDecisionHelpers.ts`](../../src/shared/engine/lineDecisionHelpers.ts) for `process_line` / `choose_line_option` decisions (legacy alias: `choose_line_reward`) and when line collapses grant `eliminate_rings_from_stack` opportunities.

- Territory (detection, borders, processing, decisions, aggregates): [`territoryDetection.ts`](../../src/shared/engine/territoryDetection.ts),
  [`territoryBorders.ts`](../../src/shared/engine/territoryBorders.ts),
  [`territoryProcessing.ts`](../../src/shared/engine/territoryProcessing.ts),
  [`territoryDecisionHelpers.ts`](../../src/shared/engine/territoryDecisionHelpers.ts) for `choose_territory_option` / `eliminate_rings_from_stack` decisions (legacy alias: `process_territory_region`), Q23 gating, and elimination bookkeeping,
  [`TerritoryAggregate.ts`](../../src/shared/engine/aggregates/TerritoryAggregate.ts).
- Placement: [`PlacementValidator.ts`](../../src/shared/engine/validators/PlacementValidator.ts),
  [`PlacementMutator.ts`](../../src/shared/engine/mutators/PlacementMutator.ts),
  [`PlacementAggregate.ts`](../../src/shared/engine/aggregates/PlacementAggregate.ts) (canonical `place_ring` / `skip_placement` validation and mutation, including no-dead-placement and skip-eligibility helpers now consumed directly by backend `RuleEngine`, `GameEngine`, and `ClientSandboxEngine`),
  [`placementHelpers.ts`](../../src/shared/engine/placementHelpers.ts).
- Victory: [`VictoryAggregate.ts`](../../src/shared/engine/aggregates/VictoryAggregate.ts),
  [`VictoryAggregate.ts`](../../src/shared/engine/aggregates/VictoryAggregate.ts).
- Turn orchestration: [`turnLogic.ts`](../../src/shared/engine/turnLogic.ts),
  [`fsm/TurnStateMachine.ts`](../../src/shared/engine/fsm/TurnStateMachine.ts),
  [`fsm/FSMAdapter.ts`](../../src/shared/engine/fsm/FSMAdapter.ts),
  [`turnOrchestrator.ts`](../../src/shared/engine/orchestration/turnOrchestrator.ts),
  plus the domain aggregates listed above.

2. Update the shared helper(s) under
   [`src/shared/engine`](../../src/shared/engine/types.ts). Avoid editing
...
3. Extend or adjust the **shared unit tests** that cover these modules:
   - Movement & captures:
     ...
     [`captureLogic.shared.test.ts`](../../tests/unit/captureLogic.shared.test.ts),
   ...
   - Lines:
     ...
     [`lineDecisionHelpers.shared.test.ts`](../../tests/unit/lineDecisionHelpers.shared.test.ts) – **canonical** line‑decision enumeration and application (`process_line`, `choose_line_option`) and when line collapses grant eliminations.
   - Territory:
     ...
     [`TerritoryAggregate.hex.feParity.test.ts`](../../tests/unit/TerritoryAggregate.hex.feParity.test.ts),
     [`territoryDecisionHelpers.shared.test.ts`](../../tests/unit/territoryDecisionHelpers.shared.test.ts) – **canonical** `choose_territory_option` / `eliminate_rings_from_stack` decision semantics (legacy alias: `process_territory_region`), including Q23 and S‑invariant bookkeeping.
   - Placement:
   ...
   - Turn sequencing & termination:
     [`OrchestratorAdapter.parity.test.ts`](../../tests/unit/OrchestratorAdapter.parity.test.ts),
     [`ClientSandboxEngine.placementParity.shared.test.ts`](../../tests/unit/ClientSandboxEngine.placementParity.shared.test.ts),
...
4. Re‑run parity suites that ensure backend and sandbox still match the shared
   ...
   - Backend vs sandbox & host engines (TypeScript): representative suites under `tests/unit/` such as
     ...
     [`Backend_vs_Sandbox.eliminationTrace.test.ts`](../../tests/parity/Backend_vs_Sandbox.eliminationTrace.test.ts),
     ...
     [`Backend_vs_Sandbox.seed1.snapshotParity.test.ts`](../../archive/tests/parity/Backend_vs_Sandbox.seed1.snapshotParity.test.ts),
     [`Backend_vs_Sandbox.seed18.snapshotParity.test.ts`](../../archive/tests/parity/Backend_vs_Sandbox.seed18.snapshotParity.test.ts),
     ...
     [`TerritoryParity.square8.R1Fixtures.test.ts`](../../tests/unit/TerritoryParity.square8.R1Fixtures.test.ts),
     [`TerritoryBorders.Backend_vs_Sandbox.test.ts`](../../tests/unit/TerritoryBorders.Backend_vs_Sandbox.test.ts),
     [`TerritoryCore.GameEngine_vs_Sandbox.test.ts`](../../tests/unit/TerritoryCore.GameEngine_vs_Sandbox.test.ts),
     [`movementReachabilityParity.test.ts`](../../tests/unit/movementReachabilityParity.test.ts),
     [`TerritoryPendingFlag.GameEngine_vs_Sandbox.test.ts`](../../tests/unit/TerritoryPendingFlag.GameEngine_vs_Sandbox.test.ts),
     [`Sandbox_vs_Backend.aiHeuristicCoverage.test.ts`](../../tests/unit/Sandbox_vs_Backend.aiHeuristicCoverage.test.ts) (heuristic AI parity, including pie-rule/swap_sides and deep seeded runs).
   - Shared engine vs hosts (TypeScript):
     [`TraceFixtures.sharedEngineParity.test.ts`](../../tests/unit/TraceFixtures.sharedEngineParity.test.ts),
     [`EngineDeterminism.shared.test.ts`](../../tests/unit/EngineDeterminism.shared.test.ts),
     [`NoRandomInCoreRules.test.ts`](../../tests/unit/NoRandomInCoreRules.test.ts).
   - Python vs TypeScript parity (AI service):
     [`generate_test_vectors.py`](../../ai-service/tests/parity/generate_test_vectors.py),
...
5. Validate **RulesMatrix** and **FAQ** scenarios under
   [`tests/scenarios`](../../tests/scenarios/FAQ_Q09_Q14.test.ts) remain green; add
   new scenario cases where appropriate (see
   [`RULES_SCENARIO_MATRIX.md`](../rules/RULES_SCENARIO_MATRIX.md) and
   [`tests/README.md`](../../tests/README.md)).

### Python Engine Structure

- [`ai-service/app/game_engine/__init__.py`](../../ai-service/app/game_engine/__init__.py): Core host adapter exposing:
...
- [`ai-service/app/rules/phase_machine.py`](../../ai-service/app/rules/phase_machine.py): Phase/turn state machine:
...
- [`ai-service/app/board_manager.py`](../../ai-service/app/board_manager.py): Board-level utilities (hashing, S-invariant, line detection, Territory regions).
- [`ai-service/app/rules/default_engine.py`](../../ai-service/app/rules/default_engine.py): Adapter that delegates to `GameEngine` while routing key move types through dedicated mutators under _shadow contracts_.
- [`ai-service/app/models/`](../../ai-service/app/models/__init__.py): Pydantic models mirroring the TypeScript shared engine types.

### TypeScript Integration

- [`src/server/game/RulesBackendFacade.ts`](../../src/server/game/RulesBackendFacade.ts): The primary entry point for the backend. It abstracts the choice between the local TS engine and the Python Service based on `RINGRIFT_RULES_MODE` while always applying moves via the canonical shared-engine–backed `GameEngine`.
- [`src/server/services/PythonRulesClient.ts`](../../src/server/services/PythonRulesClient.ts): Handles HTTP communication with the Python AI Service (`/rules/evaluate_move`).
...
- **Trace Parity:** TS-generated parity traces under `tests/fixtures/rules-parity/v1/**` together with the integration harness in [`src/server/game/test-python-rules-integration.ts`](../../src/server/game/test-python-rules-integration.ts) feed the Python parity suites.
...
| **Line Processing**         | `process_line`                                                       | `ai-service/tests/rules/test_default_engine_equivalence.py` | Synthetic line via `BoardManager.find_all_lines` monkeypatch; canonical TS semantics live in [`lineDecisionHelpers.ts`](../../src/shared/engine/lineDecisionHelpers.ts) and are enforced by [`lineDecisionHelpers.shared.test.ts`](../../tests/unit/lineDecisionHelpers.shared.test.ts)                                   |
| **Territory – Region**      | `choose_territory_option` (legacy alias: `process_territory_region`) | `ai-service/tests/rules/test_default_engine_equivalence.py` | Synthetic disconnected region via `BoardManager.find_disconnected_regions`; canonical TS semantics live in [`territoryDecisionHelpers.ts`](../../src/shared/engine/territoryDecisionHelpers.ts) and are enforced by [`territoryDecisionHelpers.shared.test.ts`](../../tests/unit/territoryDecisionHelpers.shared.test.ts) |
| **Territory – Elimination** | `eliminate_rings_from_stack`                                         | `ai-service/tests/rules/test_default_engine_equivalence.py` | Synthetic single capped stack via `_get_territory_processing_moves`; elimination bookkeeping must match the shared helpers in [`territoryDecisionHelpers.ts`](../../src/shared/engine/territoryDecisionHelpers.ts) and `TerritoryMutator`                                                                                 |
...
- Client sandbox:
  [`sandboxTerritory.ts`](../../src/client/sandbox/sandboxTerritory.ts),
  [`ClientSandboxEngine.processDisconnectedRegionsForCurrentPlayer`](../../src/client/sandbox/ClientSandboxEngine.ts).
...
- Python adapter (semantic boundary):
  [`territory.py` (TerritoryMutator)](../../ai-service/app/rules/mutators/territory.py) – Python mutator layered over the canonical TS Territory helpers/aggregate above; it is a semantic boundary, **not** an independent rules SSoT.
- Backend helpers:
  [`BoardManager.ts`](../../src/server/game/BoardManager.ts),
  [`GameEngine.ts`](../../src/server/game/GameEngine.ts) – both delegate to the
...
- A **region** is a maximal set of **non‑collapsed** cells (empty, marker, or stack) that are connected via the board’s `territoryAdjacency`. This matches RR‑CANON‑R040 and the implementation in [`territoryDetection.findDisconnectedRegions`](../../src/shared/engine/territoryDetection.ts).
- A region is **physically disconnected** when every adjacency path from any cell in the region to any non‑collapsed cell outside the region must cross only:
  ...
  - board edge, and/or
  - markers belonging to exactly one border color, as formalised in [`territoryBorders.ts`](../../src/shared/engine/territoryBorders.ts).
- Color representation (RR‑CANON‑R142) and the Q23 self‑elimination prerequisite (RR‑CANON‑R143) are handled by [`territoryProcessing.canProcessTerritoryRegion`](../../src/shared/engine/territoryProcessing.ts) and [`territoryDecisionHelpers`](../../src/shared/engine/territoryDecisionHelpers.ts); `BoardManager.findDisconnectedRegions` and the sandbox adapters now delegate to these shared helpers rather than maintaining a separate server‑local implementation.
...
- Backend:
  - [`BoardManager.ts`](../../src/server/game/BoardManager.ts)
  - [`GameEngine.ts`](../../src/server/game/GameEngine.ts)
- Python Rules Engine:
  - [`board_manager.py`](../../ai-service/app/board_manager.py)
  - [`territory.py`](../../ai-service/app/rules/mutators/territory.py)
  - [`capture.py`](../../ai-service/app/rules/mutators/capture.py)
