...
- Allowed AI types (public create-game schema): `random`, `heuristic`, `minimax`, `mcts`, `descent`, `policy_only`, `gumbel_mcts`, `ig_gmo`.
- `policy_only` and `ig_gmo` are not part of the canonical difficulty ladder; use them only via explicit AI overrides (testing/tournaments).
