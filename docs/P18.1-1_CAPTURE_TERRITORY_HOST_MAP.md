...
**Observation**: `GameSession` is largely agnostic to the *logic* of capture/territory, relying on `GameEngine` to expose the correct moves.
...
| :--- | :---: | :--- | :--- |
| [`captureSequenceEnumeration.test.ts`](../tests/unit/captureSequenceEnumeration.test.ts) | **FAIL** | **Enumeration Mismatch**: Sandbox and backend enumerate different sets of capture sequences for random square8 boards. | `enumerateSequencesSandbox` (sandboxCaptures) vs `enumerateSequencesBackend` (CaptureAggregate). |
| [`GameEngine.chainCaptureChoiceIntegration.test.ts`](../tests/unit/GameEngine.chainCaptureChoiceIntegration.test.ts) | **FAIL** | **Chain Logic Mismatch**: `enumerateChainsFrom` (test helper) expects different chains than `GameEngine` produces. | `GameEngine.getValidMoves` (chain_capture phase) vs local test DFS. |
| [`GameEngine.territoryDisconnection.test.ts`](../tests/unit/GameEngine.territoryDisconnection.test.ts) | **FAIL** | **State Update Failure**: Territory collapse expectations (collapsed spaces count) are not met after `makeMove`. | `GameEngine.makeMove` -> `processOneDisconnectedRegion` (legacy path). |
| [`RulesMatrix.Territory.ClientSandboxEngine.test.ts`](../tests/scenarios/RulesMatrix.Territory.ClientSandboxEngine.test.ts) | **FAIL** | **Missing Method**: Test calls `applyCanonicalProcessTerritoryRegion` which is missing or private. | `ClientSandboxEngine` test helpers. |
