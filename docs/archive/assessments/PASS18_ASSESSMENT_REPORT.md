...
- **Ops/SLOs and incident handling.**
  Runbooks and SLOs in `docs/runbooks` and incident docs (for example, [`docs/incidents/INDEX.md`](docs/incidents/INDEX.md:1), [`docs/operations/ALERTING_THRESHOLDS.md`](docs/operations/ALERTING_THRESHOLDS.md:1)) are well structured. The main challenge is executing the orchestrator rollout within this framework rather than designing the framework itself, so Ops is tightly coupled to the hardest problem but not itself the weakest component.
