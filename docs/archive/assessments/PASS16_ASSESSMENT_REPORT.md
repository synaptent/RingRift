...
- **Documentation & tests:** Core rules, AI, and architecture docs remain accurate overall. Spot-checks found **minor drift** in test-count and helper-status descriptions (notably the placement helpers section in [`docs/architecture/CANONICAL_ENGINE_API.md`](../../architecture/CANONICAL_ENGINE_API.md:1) and historical counts in [`PROJECT_GOALS.md`](../../../PROJECT_GOALS.md:1)), but **no materially misleading semantics**. Coverage claims in [`../historical/CURRENT_STATE_ASSESSMENT.md`](../historical/CURRENT_STATE_ASSESSMENT.md)(../historical/CURRENT_STATE_ASSESSMENT.md:236) match the current Jest and pytest suite structure.
...
- **AG – Alignment with goals:** Fit with [`PROJECT_GOALS.md`](../../../PROJECT_GOALS.md:1) and the strategic roadmap.
...
| **Python rules engine & contracts** (`ai-service/app/rules/**`, `ai-service/app/game_engine.py`) | 4.5 | 5.0 | 4.5     | 4.5 | 5.0  | 4.0 | 4.5 | **4.5**       | Mirrors TS semantics via contract vectors and parity tests; territory and forced elimination now aligned.        |
...
- **Frontend CQ = 3.5** – [`GamePage.tsx`](../../../src/client/pages/GamePage.tsx:1) and [`ClientSandboxEngine.ts`](../../../src/client/sandbox/ClientSandboxEngine.ts:1) are large, multi-responsibility files, even though they delegate rules semantics to shared helpers and the orchestrator. This concentrates UX and orchestration complexity into a small number of host components.
...
- [`GamePage.tsx`](../../../src/client/pages/GamePage.tsx:1) – primary host for backend games and local sandbox; owns layout, orchestration, error states, and many UX decisions.
- [`ClientSandboxEngine.ts`](../../../src/client/sandbox/ClientSandboxEngine.ts:1) – client-local engine harness that wires shared helpers, the orchestrator adapter [`SandboxOrchestratorAdapter`](../../../src/client/sandbox/SandboxOrchestratorAdapter.ts:1), and sandbox-specific UX semantics.
- Supporting sandbox utilities such as [`sandboxMovement.ts`](../../../src/client/sandbox/sandboxMovement.ts:1) and [`sandboxPlacement.ts`](../../../src/client/sandbox/sandboxPlacement.ts:1), which now mostly act as thin adapters over shared helpers.
...
- **File size and responsibility concentration**
  ...
  - `ClientSandboxEngine` is large (~3.6k LOC) but conceptually cleaner; it delegates rules semantics to shared helpers such as [`applySimpleMovement()`](../../../src/shared/engine/aggregates/MovementAggregate.ts:562) and [`applyCaptureSegment()`](../../../src/shared/engine/aggregates/CaptureAggregate.ts:845), yet still carries substantial host- and trace-specific logic.
...
- **Mobile and touch ergonomics**
  ...
  - While the underlying [`BoardView`](../../../src/client/components/BoardView.tsx:1) component has strong keyboard support and clear affordances, the host does not yet expose touch-friendly alternatives for all sandbox-specific flows (e.g. capture-direction choice paths) with equal clarity.
...
- **DevOps/CI enforcement** – [`WEAKNESS_ASSESSMENT_REPORT.md`](../plans/WEAKNESS_ASSESSMENT_REPORT.md:578) identified non-blocking ESLint and E2E jobs as the weakest area in Pass 7. These are now corrected; lint and Playwright suites are CI-blocking, and the Python core tests are included via dedicated jobs.
...
- **Rules documentation and SSOT discipline** – [`CURRENT_RULES_STATE.md`](../../rules/CURRENT_RULES_STATE.md:1) and [`RULES_IMPLEMENTATION_MAPPING.md`](../../rules/RULES_IMPLEMENTATION_MAPPING.md:1) accurately track the canonical rules SSoT (rules spec + shared TS engine) and test harnesses, with SSOT checks like [`rules-ssot-check.ts`](../../../scripts/ssot/rules-ssot-check.ts:1) enforcing consistency.
...
- Shared orchestrator in [`turnOrchestrator.ts`](../../../src/shared/engine/orchestration/turnOrchestrator.ts:1) with `processTurn` / `processTurnAsync` driving domain aggregates.
...
- Contract-based parity tests and v2 test vectors in both TS and Python validate cross-language semantics, as summarised in [`../historical/CURRENT_STATE_ASSESSMENT.md`](../historical/CURRENT_STATE_ASSESSMENT.md)(../historical/CURRENT_STATE_ASSESSMENT.md:71) and [`AI_ARCHITECTURE.md`](../../architecture/AI_ARCHITECTURE.md:50).
...
- **Findings:**
  - High-level goals (stable rules SSoT, AI integration, strong documentation, production readiness) remain well aligned with the current architecture and implementation described in [`../historical/CURRENT_STATE_ASSESSMENT.md`](../historical/CURRENT_STATE_ASSESSMENT.md)(../historical/CURRENT_STATE_ASSESSMENT.md:26) and [`AI_ARCHITECTURE.md`](../../architecture/AI_ARCHITECTURE.md:30).
...
- **Recommendation:** No changes required; keep referencing this document as the canonical lifecycle map alongside [`docs/architecture/CANONICAL_ENGINE_API.md`](../../architecture/CANONICAL_ENGINE_API.md:1).
...
- **Findings:**
  - [`docs/architecture/TOPOLOGY_MODES.md`](../../architecture/TOPOLOGY_MODES.md:1) correctly describes supported board topologies and references geometry helpers consistent with engine implementation and tests.
  - Runbooks such as [`docs/runbooks/RULES_PARITY.md`](../../runbooks/RULES_PARITY.md:1), [`docs/runbooks/AI_ERRORS.md`](../../runbooks/AI_ERRORS.md:1), [`docs/runbooks/AI_FALLBACK.md`](../../runbooks/AI_FALLBACK.md:1), [`docs/runbooks/AI_SERVICE_DOWN.md`](../../runbooks/AI_SERVICE_DOWN.md:1), [`docs/runbooks/GAME_HEALTH.md`](../../runbooks/GAME_HEALTH.md:1), and [`docs/runbooks/GAME_PERFORMANCE.md`](../../runbooks/GAME_PERFORMANCE.md:1) now explicitly:
...
- **Findings:**
  - [`AI_ARCHITECTURE.md`](../../architecture/AI_ARCHITECTURE.md:1) now incorporates the current canonical rules SSoT (rules spec + shared TS engine) and orchestrator-first lifecycle, as well as detailed descriptions of the AI difficulty ladder, RNG determinism, and training pipelines.
...
| -------------------------------------------------------------- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Accessibility (keyboard, ARIA, screen readers, reduced motion) | **4.6**     | `BoardView` uses grid semantics, focus management, and `aria-live` regions; `VictoryModal` is an accessible dialog with motion-safety via `prefers-reduced-motion` handling in [`globals.css`](../../../src/client/styles/globals.css:1). |
...
- **TS SSoT:** [`MovementAggregate`](../../../src/shared/engine/aggregates/MovementAggregate.ts:1) defines validation, enumeration, and mutation for non-capturing movement, delegating to shared helpers and `MovementBoardView`.
- **Python mirror:** Movement reachability and distance semantics are mirrored in Python via `BoardManager` and geometry helpers; capture-aware movement is handled in [`capture_chain.py`](../../../ai-service/app/rules/capture_chain.py:1) with consistent distance and obstruction checks.
- **Hosts:**
  - Backend [`RuleEngine`](../../../src/server/game/RuleEngine.ts:1) uses [`enumerateSimpleMoveTargetsFromStack()`](../../../src/server/game/RuleEngine.ts:223) and [`applySimpleMovementAggregate()`](../../../src/server/game/RuleEngine.ts:423) for movement validation and mutation.
  - Sandbox [`ClientSandboxEngine`](../../../src/client/sandbox/ClientSandboxEngine.ts:1) uses shared movement enumerators via [`enumerateSimpleMovementLandings`](../../../src/client/sandbox/sandboxMovement.ts:22) and applies movement through [`applySimpleMovement()`](../../../src/shared/engine/aggregates/MovementAggregate.ts:562).

### 7.2 Capture & chain capture

- **TS SSoT:** [`CaptureAggregate`](../../../src/shared/engine/aggregates/CaptureAggregate.ts:1) centralises capture validation, enumeration, mutation, and chain-capture state helpers. Validation defers to [`validateCaptureSegmentOnBoard()`](../../../src/shared/engine/core.ts:1), and enumeration uses shared movement directions and board adapters.
- **Python mirror:** [`capture_chain.py`](../../../ai-service/app/rules/capture_chain.py:1) implements Python analogues such as [`validate_capture_segment_on_board_py()`](../../../ai-service/app/rules/capture_chain.py:92), [`enumerate_capture_moves_py()`](../../../ai-service/app/rules/capture_chain.py:213), and [`apply_capture_py()`](../../../ai-service/app/rules/capture_chain.py:448), delegating final mutation to the Python [`GameEngine`](ai-service/app/game_engine.py:1).
- **Hosts:**
  - Backend [`RuleEngine.processCapture()`](../../../src/server/game/RuleEngine.ts:444) is a thin adapter over [`applyCaptureAggregate()`](../../../src/server/game/RuleEngine.ts:445), with chain continuation determined via shared continuation helpers.
  - Sandbox `ClientSandboxEngine` uses shared capture helpers via [`applyCaptureSegmentAggregate()`](../../../src/client/sandbox/ClientSandboxEngine.ts:1510) and chain-continuation checks via [`getChainCaptureContinuationInfoAggregate()`](../../../src/client/sandbox/ClientSandboxEngine.ts:1542).

### 7.3 Placement & no-dead-placement

- **TS SSoT:** [`PlacementAggregate`](../../../src/shared/engine/aggregates/PlacementAggregate.ts:1) exposes [`validatePlacementOnBoard()`](../../../src/shared/engine/aggregates/PlacementAggregate.ts:158), [`enumeratePlacementPositions()`](../../../src/shared/engine/aggregates/PlacementAggregate.ts:465), and placement mutators, relying on shared helpers such as `hasAnyLegalMoveOrCaptureFromOnBoard` to enforce no-dead-placement.
- **Python mirror:** [`placement.py`](../../../ai-service/app/rules/placement.py:1) defines parallel structures – [`PlacementContextPy`](../../../ai-service/app/rules/placement.py:33), [`validate_placement_on_board_py()`](../../../ai-service/app/rules/placement.py:99), and [`apply_place_ring_py()`](../../../ai-service/app/rules/placement.py:350) – delegating low-level mutation and hypothetical placement checks to the Python `GameEngine`.
- **Hosts:**
  - Backend `RuleEngine` calls [`validatePlacementOnBoardAggregate()`](../../../src/server/game/RuleEngine.ts:170) and [`applyPlacementMoveAggregate()`](../../../src/server/game/RuleEngine.ts:403) for validation and mutation during `ring_placement`.
  - Sandbox `ClientSandboxEngine` routes both human and AI placements through shared helpers, notably [`validatePlacementAggregate()`](../../../src/client/sandbox/ClientSandboxEngine.ts:2985), [`applyPlacementMoveAggregate()`](../../../src/client/sandbox/ClientSandboxEngine.ts:3002), and enumerators such as [`enumeratePlacementPositions()`](../../../src/client/sandbox/ClientSandboxEngine.ts:1093).

### 7.4 Residual duplication and risk

- Legacy sandbox helpers such as [`sandboxMovement.ts`](../../../src/client/sandbox/sandboxMovement.ts:1) and [`sandboxPlacement.ts`](../../../src/client/sandbox/sandboxPlacement.ts:1) now act as **thin adapters** over shared helpers and are used primarily for UX and hypothetical-board tooling, not as independent semantics sources.
- Backend `RuleEngine` retains some historical helpers (e.g. line and territory post-processing methods), but decision surfaces for lines, territory, and elimination now defer to shared helpers like [`enumerateProcessLineMoves()`](../../../src/shared/engine/lineDecisionHelpers.ts:1) and [`enumerateTerritoryEliminationMoves()`](../../../src/shared/engine/territoryDecisionHelpers.ts:1).
- SSOT tooling such as [`rules-ssot-check.ts`](../../../scripts/ssot/rules-ssot-check.ts:1) enforces mapping consistency between [`RULES_CANONICAL_SPEC.md`](../../../RULES_CANONICAL_SPEC.md:1) and [`RULES_IMPLEMENTATION_MAPPING.md`](../../rules/RULES_IMPLEMENTATION_MAPPING.md:1), reducing the risk of silent divergence.
...
| ---------------- | ----------------------------------------------------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **P16.1-ARCH**   | Finalise orchestrator-first migration plan                        | Architect | This PASS16 report; [`WEAKNESS_ASSESSMENT_REPORT.md`](../plans/WEAKNESS_ASSESSMENT_REPORT.md:229); [`docs/drafts/RULES_ENGINE_CONSOLIDATION_DESIGN.md`](../RULES_ENGINE_CONSOLIDATION_DESIGN.md:1); [`docs/architecture/ORCHESTRATOR_ROLLOUT_PLAN.md`](../../architecture/ORCHESTRATOR_ROLLOUT_PLAN.md:1) | **(Largely complete in this pass.)** `docs/architecture/ORCHESTRATOR_ROLLOUT_PLAN.md` now defines phases, flags, CI profiles, metrics, and rollback levers for orchestrator-first rollout; remaining architectural decisions relate to concrete legacy deletions and any future SLO tweaks, to be driven by P16.6–P16.8.                                                       |
| **P16.2-CODE**   | Decompose `GamePage` into backend and sandbox hosts               | Code      | [`GamePage.tsx`](../../../src/client/pages/GamePage.tsx:1); [`ClientSandboxEngine.ts`](../../../src/client/sandbox/ClientSandboxEngine.ts:1); PASS16 scorecard                                                                                                                                   | `GamePage.tsx` reduced to < 800 LOC with separate `BackendGameHost` and `SandboxGameHost` components; all existing Jest and Playwright suites pass; no behavioural regressions in manual smoke tests.                                                                                                                                                             |
