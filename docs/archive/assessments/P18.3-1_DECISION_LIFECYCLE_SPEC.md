...
- [`routes/game.ts`](../../../src/server/routes/game.ts)
- [`choice` state machine](../../../src/shared/stateMachines/choice.ts)
- [`connection` state machine](../../../src/shared/stateMachines/connection.ts)
- [`GameContext`](../../../src/client/contexts/GameContext.tsx)
- Invariants in [`INVARIANTS_AND_PARITY_FRAMEWORK.md`](../../rules/INVARIANTS_AND_PARITY_FRAMEWORK.md)
...
- No violation of `INV-ACTIVE-NO-MOVES`: an `active` game must never expose a state where the current player has no legal global action (placement, movement, capture, or required decision). The safety net in [`GameEngine.getValidMoves()`](../../../src/server/game/GameEngine.ts) and [`GameEngine.resolveBlockedStateForCurrentPlayerForTesting()`](../../../src/server/game/GameEngine.ts) remains authoritative.
- No violation of `INV-PHASE-CONSISTENCY`: completed games must not be left in bookkeeping or decision phases. [`GameEngine.endGame()`](../../../src/server/game/GameEngine.ts) normalises `currentPhase` to `ring_placement`.
...
- Compatibility with orchestrator-first hosting: decision phases are expressed as canonical `Move` instances via [`GameEngine.getValidMoves()`](../../../src/server/game/GameEngine.ts) and processed through [`GameEngine.makeMove()`](../../../src/server/game/GameEngine.ts) / [`GameEngine.makeMoveById()`](../../../src/server/game/GameEngine.ts), regardless of whether the underlying rules logic is provided by the legacy engine or the shared orchestrator adapter.
...
Canonical game status is carried on [`GameState.gameStatus`](../../../src/shared/types/game.ts) and projected into a derived [`GameSessionStatus`](../../../src/shared/stateMachines/gameSession.ts) by [`GameSession.recomputeSessionStatus()`](../../../src/server/game/GameSession.ts).

| Logical state | GameState.gameStatus | SessionStatus.kind (typical)             | Owner                                                                                                                                                              | Notes                                                                                                                                            |
| ------------- | -------------------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| Waiting       | `waiting`            | `waiting_for_players`                    | [`GameEngine`](../../../src/server/game/GameEngine.ts) / DB                                                                                                        | Game created but not started; no decisions or timeouts are active.                                                                               |
| Active        | `active`             | `active_turn` or `active_decision_phase` | [`GameEngine`](../../../src/server/game/GameEngine.ts) / [`GameSession`](../../../src/server/game/GameSession.ts)                                                  | Normal play; all decision/timeout semantics in this spec apply.                                                                                  |
| Paused        | `paused`             | `paused`                                 | [`GameEngine.pauseGame()`](../../../src/server/game/GameEngine.ts)                                                                                                 | Clocks/timers are paused; decision-phase timers must be cleared.                                                                                 |
| Completed     | `completed`          | `completed`                              | [`GameEngine.endGame()`](../../../src/server/game/GameEngine.ts) / [`GamePersistenceService.finishGame()`](../../../src/server/services/GamePersistenceService.ts) | Terminal win/draw/forfeit result with a concrete [`GameResult`](../../../src/shared/types/game.ts). Rating updates apply when `isRated` is true. |
| Abandoned     | `abandoned`          | `abandoned`                              | DB / [`GameSession.terminate()`](../../../src/server/game/GameSession.ts)                                                                                          | Terminal non-competitive end (e.g., cancelled lobby game, fatal host error). Rating should not change.                                           |

Constraints:

- Decision-phase timers in [`GameSession`](../../../src/server/game/GameSession.ts) MUST NOT be active when `gameStatus !== 'active'`.
- `gameStatus === 'completed'` or `abandoned` implies no further player moves, decisions, or rating changes.

### 2.2 Turn and phase

Rules-level state for whose turn it is and what part of the rules pipeline is active is carried by [`GameState.currentPlayer`](../../../src/shared/types/game.ts) and [`GameState.currentPhase`](../../../src/shared/types/game.ts), with the allowed values defined by [`GamePhase`](../../../src/shared/types/game.ts).
...
Phase transitions are managed by [`GameEngine.advanceGame()`](../../../src/server/game/GameEngine.ts) and, in orchestrator-backed flows, by the shared turn engine wrapped in [`TurnEngineAdapter`](../../../src/server/game/turn/TurnEngineAdapter.ts) via [`GameEngine.processMoveViaAdapter()`](../../../src/server/game/GameEngine.ts).

Invariants:

- Interactive phases must satisfy `INV-ACTIVE-NO-MOVES`: [`GameEngine.getValidMoves()`](../../../src/server/game/GameEngine.ts) will invoke [`GameEngine.resolveBlockedStateForCurrentPlayerForTesting()`](../../../src/server/game/GameEngine.ts) if no moves are available while `gameStatus === 'active'`.
- Decision phases must never be observable with zero canonical decision moves for the current player. When no decision moves exist, [`GameEngine.stepAutomaticPhasesForTesting()`](../../../src/server/game/GameEngine.ts) or turn-advancement must transition to the next phase or end the game.
...
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `NO_DECISION_PENDING`            | `gameStatus !== 'active'` OR `currentPhase` not in `{line_processing, territory_processing, chain_capture}` OR no decision moves for current human player                                                                                                                                                                        | [`GameEngine`](../../../src/server/game/GameEngine.ts)   | Enters when game starts, after decisions resolve, or for AI turns.                                                                       |
| `DECISION_PENDING`               | `gameStatus === 'active'`; `currentPhase` ∈ `{line_processing, territory_processing, chain_capture}`; current player is human; `GameEngine.getValidMoves()` exposes one or more decision-type `Move`s; [`GameSession.scheduleDecisionPhaseTimeout()`](../../../src/server/game/GameSession.ts) has set `decisionTimeout*` fields | [`GameSession`](../../../src/server/game/GameSession.ts) | Entered after any move that yields a decision surface; leaves on move application, phase change, game end, or explicit timeout handling. |
| `DECISION_TIMED_OUT` (ephemeral) | Internal moment when `decisionTimeoutHandle` fires and [`GameSession.handleDecisionPhaseTimedOut()`](../../../src/server/game/GameSession.ts) begins executing                                                                                                                                                                   | [`GameSession`](../../../src/server/game/GameSession.ts) | Immediately transitions to `DECISION_AUTO_RESOLVED` or back to `NO_DECISION_PENDING` if guard checks fail.                               |

| `DECISION_AUTO_RESOLVED` | A canonical `Move` has been auto-selected and applied on behalf of the player, and [`DecisionAutoResolvedMeta`](../../../src/shared/types/websocket.ts) has been attached to the last move | [`GameSession`](../../../src/server/game/GameSession.ts) / [`GamePersistenceService`](../../../src/server/services/GamePersistenceService.ts) | Visible to clients via `game_state.data.meta.diffSummary.decisionAutoResolved` and to history via `moveData.decisionAutoResolved`; next state is recomputed based on resulting `GameState`. |

Guards in [`GameSession.handleDecisionPhaseTimedOut()`](../../../src/server/game/GameSession.ts) enforce that auto-resolution only occurs when:
...
Per-player connection lifecycle is modelled by [`PlayerConnectionState`](../../../src/shared/stateMachines/connection.ts) and stored in [`WebSocketServer.playerConnectionStates`](../../../src/server/websocket/server.ts).
...
| `CONNECTED` | Player's WebSocket is currently joined to the game room. | [`WebSocketServer.handleJoinGame()`](../../../src/server/websocket/server.ts) calls [`markConnected()`](../../../src/shared/stateMachines/connection.ts) whenever a socket with `userId` joins a game. |
| `DISCONNECTED_PENDING_RECONNECT` | Player's socket disconnected from a game where they are a seated player; reconnection timeout window is active. | [`WebSocketServer.handleDisconnect()`](../../../src/server/websocket/server.ts) calls [`markDisconnectedPendingReconnect()`](../../../src/shared/stateMachines/connection.ts) and schedules a timeout. |
| `DISCONNECTED_EXPIRED` | Player did not reconnect before `RECONNECTION_TIMEOUT_MS`; reconnect window expired. | [`WebSocketServer.handleReconnectionTimeout()`](../../../src/server/websocket/server.ts) calls [`markDisconnectedExpired()`](../../../src/shared/stateMachines/connection.ts). |
...
- On `DISCONNECTED_EXPIRED`: all pending transport-level choices for that player are canceled via [`WebSocketInteractionHandler.cancelAllChoicesForPlayer()`](../../../src/server/game/WebSocketInteractionHandler.ts). The game itself is **not** ended or forfeited today.
...
- **Decision-phase timeouts** (per decision surface; owned by [`GameSession`](../../../src/server/game/GameSession.ts)).
- **PlayerChoice timeouts** (per transport choice; owned by [`WebSocketInteractionHandler`](../../../src/server/game/WebSocketInteractionHandler.ts)).
- **Move-clock timeouts** (per player timer; owned by [`GameEngine`](../../../src/server/game/GameEngine.ts)).
- **Reconnect window timeouts** (per player; owned by [`WebSocketServer`](../../../src/server/websocket/server.ts)).

### 3.1 Decision-phase timeouts (line, territory, chain capture)

Decision-phase timeouts are configured via `config.decisionPhaseTimeouts` and scheduled by [`GameSession.scheduleDecisionPhaseTimeout()`](../../../src/server/game/GameSession.ts) whenever `GameState` indicates a decision surface for the active human player in one of:
...
- **Warning:** if `warningBeforeMs > 0`, [`GameSession.emitDecisionPhaseTimeoutWarning()`](../../../src/server/game/GameSession.ts) is scheduled at `timeoutMs - warningBeforeMs`, emitting a `decision_phase_timeout_warning` event with remaining time.
- **Stop/reset:** `resetDecisionPhaseTimeout()` is called whenever a new state is broadcast via [`GameSession.broadcastUpdate()`](../../../src/server/game/GameSession.ts) or a terminal `game_over` is emitted. Any subsequent decision surface will reschedule a fresh timeout.
...
UI note: Client HUD and dialog countdowns consume these millisecond deadlines and warnings and render whole-second values using ceil semantics via [`msToDisplaySeconds()`](../../../src/client/utils/countdown.ts); these countdowns are advisory only, with authoritative expiry still defined by the guards in [`GameSession.handleDecisionPhaseTimedOut()`](../../../src/server/game/GameSession.ts) and the resulting `game_state` transitions.

#### 3.1.2 Behaviour on timeout

When the deadline elapses, [`GameSession.handleDecisionPhaseTimedOut()`](../../../src/server/game/GameSession.ts) executes the following algorithm:
...
5. Apply the move via [`RulesBackendFacade.applyMoveById()`](../../../src/server/game/RulesBackendFacade.ts); on success:
   - Attach a [`DecisionAutoResolvedMeta`](../../../src/shared/types/websocket.ts) record to the last move in `moveHistory`.
   - Persist the move via [`GameSession.persistMove()`](../../../src/server/game/GameSession.ts).
   ...
   - Invoke [`GameSession.maybePerformAITurn()`](../../../src/server/game/GameSession.ts) if AI is next.
...
Transport-level `PlayerChoice` flows (e.g., legacy line-order prompts, ring-elimination dialogs in non-move-driven mode) are owned by [`WebSocketInteractionHandler`](../../../src/server/game/WebSocketInteractionHandler.ts).

- When `requestChoice()` is called, a `pending` [`ChoiceStatus`](../../../src/shared/stateMachines/choice.ts) is created with `deadlineAt = now + timeoutMs` (default 30s), and `player_choice_required` is emitted.
- If the client responds with `player_choice_response` before the deadline and the response passes validation, the choice becomes `fulfilled` via [`markChoiceFulfilled()`](../../../src/shared/stateMachines/choice.ts).
- If the deadline elapses first, the choice becomes `expired` via [`markChoiceExpired()`](../../../src/shared/stateMachines/choice.ts) and the Promise is rejected. No game move is auto-applied at this layer.
...
- `GameSession.maybePerformAITurn()`:
  - Derives a per-session AI timeout budget from `config.aiService.requestTimeoutMs`
    (see `AI_SERVICE_REQUEST_TIMEOUT_MS` in `docs/operations/ENVIRONMENT_VARIABLES.md`).
  ...
  - Wraps `globalAIEngine.getAIMove(playerNumber, state)` in
    [`runWithTimeout`](../../../src/shared/utils/timeout.ts) via
...
Per-player move clocks are implemented in [`GameEngine.startPlayerTimer()`](../../../src/server/game/GameEngine.ts) / [`GameEngine.stopPlayerTimer()`](../../../src/server/game/GameEngine.ts):

- For non-AI players, a Node timer is started with duration `player.timeRemaining` when the player's interactive turn begins.
- On expiry, [`GameEngine.forfeitGame()`](../../../src/server/game/GameEngine.ts) is invoked, which in turn calls `endGame(winner, 'resignation')`.
...
Reconnect windows are owned by [`WebSocketServer`](../../../src/server/websocket/server.ts) and tracked in `pendingReconnections`.
...
- If the player calls `join_game` for the same game within the window, `handleJoinGame()` cancels the timeout, calls [`markConnected()`](../../../src/shared/stateMachines/connection.ts), and emits `player_reconnected` with the player number.
- If the timer elapses first, [`handleReconnectionTimeout()`](../../../src/server/websocket/server.ts) marks the state as `DISCONNECTED_EXPIRED`, cancels any pending choices for that player via `cancelAllChoicesForPlayer()`, and emits no further game events.
...
- HTTP `POST /games/:gameId/leave` on an **active** game (see [`routes/game.ts`](../../../src/server/routes/game.ts)):
...
- Clean resign SHOULD be expressed as a game-level forfeit processed by the active host:
  - The canonical flow is to call a dedicated resign API that resolves to [`GameEngine.endGame()`](../../../src/server/game/GameEngine.ts) with `reason: 'resignation'` and `winner` set to the opposing player.
...
- When a player's move-clock timer expires, [`GameEngine.forfeitGame()`](../../../src/server/game/GameEngine.ts) calls `endGame(winner, 'timeout')`, producing a `GameResult` with `reason === 'timeout'`.
...
- When a player disconnects and fails to reconnect within `RECONNECTION_TIMEOUT_MS`, [`handleReconnectionTimeout()`](../../../src/server/websocket/server.ts):
...
- UX and telemetry SHOULD surface which decisions were auto-resolved both in real time (`decision_phase_timed_out`, `game_state.meta.diffSummary.decisionAutoResolved`) and via history ([`/games/:id/history`](../../../src/server/routes/game.ts)).
...
| Move-clock timeout                                        | Move-clock expiry in [`startPlayerTimer()`](../../../src/server/game/GameEngine.ts) (target: reason `'timeout'`)      | `'timeout'`                                    | Opponent              | Rating updates applied.                                      |
...
The shared orchestrator is integrated via [`TurnEngineAdapter`](../../../src/server/game/turn/TurnEngineAdapter.ts) and used by [`GameEngine.processMoveViaAdapter()`](../../../src/server/game/GameEngine.ts). Decision lifecycle semantics in this spec are host-level and MUST hold regardless of whether the adapter or the legacy pipeline produced the underlying `GameState` transition.

Key integration points:

- All player-facing decisions in decision phases are expressed as canonical `Move`s surfaced by [`GameEngine.getValidMoves()`](../../../src/server/game/GameEngine.ts) with `useMoveDrivenDecisionPhases` enabled via [`GameEngine.enableMoveDrivenDecisionPhases()`](../../../src/server/game/GameEngine.ts).
- `player_move_by_id` over WebSockets is the canonical transport for selecting these moves; [`WebSocketServer.handlePlayerMoveById()`](../../../src/server/websocket/server.ts) delegates to [`GameSession.handlePlayerMoveById()`](../../../src/server/game/GameSession.ts), which in turn calls [`RulesBackendFacade.applyMoveById()`](../../../src/server/game/RulesBackendFacade.ts).
- Decision-phase timeouts in [`GameSession`](../../../src/server/game/GameSession.ts) operate purely on canonical Moves and do not depend on the underlying rules engine implementation.
- Orchestrator-specific decision points (for example, `elimination_target` surfaced through [`DecisionHandler`](../../../src/server/game/GameEngine.ts)) either:
...
- **Current (2025‑12‑01):**
  - [`POST /games/:gameId/leave`](../../../src/server/routes/game.ts) now prefers to route active-game resignations through the host:
    - When a `WebSocketServer` instance is attached, the route calls
      `wsServer.handlePlayerResignFromHttp(gameId, userId)`, which:
      - Acquires the per-game lock via [`GameSessionManager.withGameLock`](../../../src/server/game/GameSessionManager.ts),
      - Invokes [`GameSession.handlePlayerResignationByUserId`](../../../src/server/game/GameSession.ts),
      - Calls [`GameEngine.resignPlayer`](../../../src/server/game/GameEngine.ts) → `endGame(winner, 'resignation')`, and
      - Finishes the game via [`GamePersistenceService.finishGame`](../../../src/server/services/GamePersistenceService.ts).
    - For environments without an attached `WebSocketServer` (for example, some unit tests), the route falls back to the legacy
      DB-only behaviour: it infers a winner from the remaining seated players, calls [`RatingService.processGameResult`](../../../src/server/services/RatingService.ts)
...
- **Current:** [`GameEngine.forfeitGame()`](../../../src/server/game/GameEngine.ts) calls `endGame(winner, 'timeout')` when invoked by move-clock expiry, so `GameResult.reason === 'timeout'` for clock-based forfeits.
...
- **Current (2025‑12‑01):**
  - [`WebSocketServer.handleReconnectionTimeout()`](../../../src/server/websocket/server.ts) now:
    - Marks the connection state as `DISCONNECTED_EXPIRED`,
    ...
    - Under the per-game lock (`withGameLock`), inspects the current `GameState` and:
      - When the game is rated and at least one other human remains connected or within their own reconnect window, calls
        [`GameSession.handleAbandonmentForDisconnectedPlayer(playerNumber, true)`](../../../src/server/game/GameSession.ts), which:
        - Uses [`GameEngine.abandonPlayer`](../../../src/server/game/GameEngine.ts) → `endGame(winner, 'abandonment')`,
      ...
      - When the game is unrated or all human players have `DISCONNECTED_EXPIRED`, calls
        ...
        [`GameEngine.abandonGameAsDraw`](../../../src/server/game/GameEngine.ts) to mark the game as abandoned without a specific winner.
...
- **Current:** [`GameSession.handleDecisionPhaseTimedOut()`](../../../src/server/game/GameSession.ts) selects the first decision candidate in the raw `getValidMoves()` list, whose ordering is not documented as stable across hosts and engines.
...
- **Current:** Decision-phase timers in [`GameSession`](../../../src/server/game/GameSession.ts) are independent of connection state; a disconnected player's decision may be auto-resolved while still within the reconnect window, and reconnect timeout later only cancels transport choices.
...
- **Current:** `endGame()` normalises `currentPhase` to `ring_placement`, but some legacy flows (e.g., test-only automatic territory processing in [`GameEngine.stepAutomaticPhasesForTesting()`](../../../src/server/game/GameEngine.ts)) may momentarily leave `gameStatus !== 'active'` with `currentPhase === 'territory_processing'` before final cleanup.
...
- PASS18 planning docs:
  - [`PASS18_REMEDIATION_PLAN.md`](PASS18_REMEDIATION_PLAN.md)
  - [`PASS18_WORKING_NOTES.md`](PASS18_WORKING_NOTES.md)
    will reference this file as the SSoT for decision lifecycle semantics.
- Related host/rules specs:
  - [`P18.1-1_CAPTURE_TERRITORY_HOST_MAP.md`](P18.1-1_CAPTURE_TERRITORY_HOST_MAP.md)
  - [`P18.2-1_AI_RNG_PATHS.md`](P18.2-1_AI_RNG_PATHS.md)
