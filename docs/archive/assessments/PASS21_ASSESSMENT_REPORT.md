...
#### Follow-up Status (PASS24.1)

Since this PASS21 assessment, PASS24.1 has executed all four k6 load-test scenarios against the nginx-fronted stack. The HTTP availability and WebSocket handshake stability issues originally called out in this section have been addressed by removing the direct host mapping of the app container on port 3000 and routing all load-test traffic through nginx on port 80 (`BASE_URL=http://127.0.0.1`, `WS_URL=ws://127.0.0.1`). All four scenarios now run to completion without socket-level `ECONNREFUSED` or `status=0` connection failures.

**Current status relative to the P21.4-2 production-scale load-test objectives:**

- **Infra / availability:** Acceptable under load for both HTTP and WebSocket:
  - Health (`/health`) and auth (`/auth/login`) endpoints remain reachable throughout the runs.
  - WebSocket handshakes to `/socket.io/` succeed at &gt;99% and are no longer a source of connection-level failures.
- **Application-level gaps (still open):**
  - `GET /api/games/:gameId` contract / ID behaviour in the concurrent-games and player-moves scenarios (high 4xx rates due to invalid or short-lived IDs under k6’s assumptions).
  - WebSocket protocol alignment in the `websocket-stress` scenario (server closes connections quickly with message-parse errors despite 100% handshake success).
  - SLO / threshold tuning for error rates and connection duration now that infra availability is stable.
