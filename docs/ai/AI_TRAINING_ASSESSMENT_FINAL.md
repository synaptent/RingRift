...
> - **Rules semantics SSoT:** Canonical rules spec (`RULES_CANONICAL_SPEC.md` together with `ringrift_complete_rules.md` / `ringrift_compact_rules.md`) as the single source of truth for rules semantics, with the shared TypeScript rules engine under `src/shared/engine/**` (helpers → aggregates → orchestrator → contracts) together with v2 contract vectors under `tests/fixtures/contract-vectors/v2/**` and the lifecycle/API SSoT in [`docs/architecture/CANONICAL_ENGINE_API.md`](../architecture/CANONICAL_ENGINE_API.md) as its primary executable implementation.
