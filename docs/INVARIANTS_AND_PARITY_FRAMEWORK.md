...
- For every *reachable* state `state` produced by the TS or Python engines, at every **invariant checkpoint** (post shared-engine `processTurn` / Python `GameEngine.apply_move`, after any automatic phase-exit / forced-elimination chain, and at soak snapshot boundaries):
...
- The **only** allowed shape in which *all* players have no global legal actions is the global stalemate of [`RR-CANON-R173`](RULES_CANONICAL_SPEC.md:551), which must be resolved immediately into a terminal state. Engines should not surface such a state to hosts or soaks as `gameStatus == ACTIVE`.
...
| `PARITY-BACKEND-SANDBOX-TS` | `orchestrator-parity` | Backend vs sandbox orchestrator multi‑phase suites. |
