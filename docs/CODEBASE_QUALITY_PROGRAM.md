...
- The live distribution docs are now being tightened too: `RESILIENT_TRANSFER_GUIDE.md`, `MODEL_LIFECYCLE.md`, `runbooks/cluster_deployment.md`, and `DAEMON_REGISTRY.md` now describe the current unified distribution surface (`DistributionConfig`, `get_distribution_daemon`, and the `MODEL_DISTRIBUTION` runner backed by `UnifiedDistributionDaemon`) instead of teaching removed `ModelDistributionDaemon`/`NPZDistributionDaemon` class names as the active path.
- The remaining distribution migration notes are now being aligned too: `STRATEGIC_IMPROVEMENT_PLAN_2025_12.md` now names `UnifiedDistributionDaemon` directly, and `DEPRECATED_MODULES_MIGRATION.md` now shows the real event-driven unified-distribution accessors and model-availability helpers instead of a nonexistent imperative `daemon.distribute(...)` API.
- The lingering data-events planning/deprecation references are now being updated too: `DEPRECATION_TRACKER.md` and `planning/INTEGRATION_ASSESSMENT_DEC2025.md` now treat `app.distributed.data_events` as a package (`event_types.py`, `event_bus.py`, `emit.py`) instead of describing the removed `app/distributed/data_events.py` single-file layout.
- The experimental-AI docs are now being tightened too: `EXPERIMENTAL_AI.md`, `training/EXPERIMENTAL_ALGORITHMS.md`, and `EBMO_RESULTS.md` now point at the real EBMO online-learning module (`ebmo_online_learner.py`), the archived CAGE module paths under `app.ai.archive`, the correct GMO class name (`GMOAI`), and the actual GPU minimax module (`gpu_minimax_ai.py`) instead of stale filenames and imports.
- The live parity/debugging docs are now being aligned with the actual rules package layout too: `runbooks/PARITY_GATE_RESOLUTION.md`, `runbooks/PARITY_MISMATCH_DEBUG.md`, and `architecture/ARCHITECTURE_OVERVIEW.md` now point engineers at `app/board_manager.py`, `app/rules/default_engine.py`, `app/rules/global_actions.py`, the territory/capture generator-mutator-validator modules, and the real TS engine files instead of removed single-file paths like `app/rules/territory.py`, `app/rules/capture.py`, or `app/rules/phase_transitions.py`.
- The remaining live neural-net reference docs are now being aligned with the package facade too: `architecture/NEURAL_AI_ARCHITECTURE.md`, `app/ai/neural_net_analysis.md`, `app/ai/__init__.py`, `training/generate_data.py`, and `coordination/tournament_daemon.py` now point at the `app.ai.neural_net` package surface, `hex_architectures.py`, `model_factory.py`, and `unified_factory.py` instead of describing removed `neural_ai.py` / `neural_net.py` single-file modules.
- The remaining distribution and model-loading reference docs are now being aligned with the live package surfaces too: `CLUSTER_INTEGRATION_GUIDE.md` and `DEPRECATION_ROADMAP.md` now point at `app/coordination/unified_distribution_daemon.py` plus `create_unified_distribution_daemon()` and the archived `_deprecated_*distribution_daemon.py` paths, while `planning/INTEGRATION_ASSESSMENT_DEC2025.md` now describes the real AI loading split (`app.ai.neural_net.model_factory` + `app.ai.unified_loader`) instead of proposing a nonexistent `app/ai/model_loading.py`.
...
- The remaining event migration docs now make that compatibility split explicit too: `app/events/README.md`, `EVENT_NAMING_CONVENTION.md`, `EVENT_NORMALIZATION_SUMMARY.md`, and `EVENT_NORMALIZATION_EXAMPLES.md` now call out `StageEvent.TRAINING_COMPLETE` / `RingRiftEventType.STAGE_TRAINING_COMPLETE` as legacy stage-bus aliases, while routing new subscriptions and public guidance to the canonical `TRAINING_COMPLETED` event name.
- The last live single-file `data_events.py` references are being drained too: `app/events/README.md` and the `HandlerBase._get_event_subscriptions()` docstring now point engineers at the `app.distributed.data_events` package / `event_types.py` instead of the removed `app/distributed/data_events.py` file, and the docs ratchet now locks that package-path guidance.
- The remaining code-adjacent `TRAINING_COMPLETE` notes now say what they actually are too: `app/training/background_eval.py`, `app/coordination/stage_events.py`, `app/coordination/event_emitters.py`, and `app/coordination/pipeline_actions.py` now mark `TRAINING_COMPLETE` as a legacy stage-event alias while routing public/router guidance to `TRAINING_COMPLETED`, and the docs ratchet now locks that compatibility wording.
...
- `app.coordination.__init__` is now back to an actual entrypoint instead of a wrapper barnacle: it is down from `251` lines to `81`, and the remaining lifecycle/status/core-module compatibility hooks are all resolved through one lazy alias table with a ratchet test keeping the file under `100` lines.
- `app.distributed.__init__` now states an actual removal target for its last lazy deprecated cluster-coordinator symbols, and the package-export test now asserts those warnings include both the migration path and the `Q3 2026` retirement timeline.
- The `# type: ignore` budget contract is now calibrated to the live count too: `app/` currently sits at `199`, and the contract test now locks future growth to a narrow `+5` margin instead of drifting on an older baseline.
