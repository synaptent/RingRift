...
| Credibility             | `8.6` |  `9.0` | The public docs are materially more honest: README, `docs/RESULTS.md`, `docs/REPRODUCIBILITY.md`, and `docs/LESSONS_LEARNED.md` use real results and caveats instead of vague claims. Stale root clutter and generated artifacts were removed, the supported documentation path now has a local-link ratchet, and result claims now have an evidence manifest separating repo-verifiable, S3-backed, and operator-reported data. Secret scans found placeholders/examples/test fixtures, not private keys or real provider tokens. Current checked-in results still lag the operator-reported newest `square8_2p` promotion to about `1782` Elo, so claims remain conservative until evidence is checked in. |
