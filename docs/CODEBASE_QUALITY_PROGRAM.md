...
| Credibility             | `8.5` |  `9.0` | The public docs are materially more honest: README, `docs/RESULTS.md`, `docs/REPRODUCIBILITY.md`, and `docs/LESSONS_LEARNED.md` use real results and caveats instead of vague claims. Stale root clutter and generated artifacts were removed, the supported documentation path now has a local-link ratchet, and `npm install` generates Prisma before server startup. Secret scans found placeholders/examples/test fixtures, not private keys or real provider tokens. Current checked-in results still lag the operator-reported newest `square8_2p` promotion to about `1782` Elo, so claims remain conservative until evidence is checked in. |
...
1. Add an evidence manifest for the newest training promotions once durable
   metrics/log artifacts are checked in or referenced, then refresh
   `docs/RESULTS.md` and `docs/data/results_snapshot.json`.
2. Continue reducing the remaining `20` direct-unimported `app.*` candidates
   after confirming dynamic-entrypoint use.
3. Add a local operational preflight checker that validates the checked-in
   fleet manifest/runbook prerequisites without touching remote training nodes.
