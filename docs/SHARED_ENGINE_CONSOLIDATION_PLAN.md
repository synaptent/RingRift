...
| Canonical orchestrator (WIP) | [`orchestration/turnOrchestrator.ts`](src/shared/engine/orchestration/turnOrchestrator.ts)                                                                                                                                                 | ~400              | Canonical `processTurn` orchestration, currently partially wired via [`TurnEngineAdapter`](src/server/game/turn/TurnEngineAdapter.ts) and [`SandboxOrchestratorAdapter.ts`](src/client/sandbox/SandboxOrchestratorAdapter.ts) |
...
**Agent:** Architect  
**Input Dependencies:** [`RULES_ENGINE_CONSOLIDATION_DESIGN.md`](docs/drafts/RULES_ENGINE_CONSOLIDATION_DESIGN.md), existing [`CaptureAggregate.ts`](src/shared/engine/aggregates/CaptureAggregate.ts), TS/Python parity tests under [`tests/parity`](ai-service/tests/parity/)
...
- [x] Updated design section in [`RULES_ENGINE_CONSOLIDATION_DESIGN.md`](docs/drafts/RULES_ENGINE_CONSOLIDATION_DESIGN.md) referencing the final capture-chain spec.
...
- Legacy helpers now acting as shims:
  ...
  - [`captureChainHelpers.ts`](src/shared/engine/captureChainHelpers.ts)
    - Refactored into a compatibility layer that:
      - Defines a legacy [`ChainCaptureStateSnapshot`](src/shared/engine/captureChainHelpers.ts:41) with `visitedTargets?: string[]` and maps it to the canonical snapshot’s `capturedThisChain: Position[]` via `stringToPosition`.
      - Adapts [`ChainCaptureEnumerationOptions`](src/shared/engine/captureChainHelpers.ts:68) to the aggregate’s `ChainCaptureEnumerationOptions`, with explicit property copying to keep `exactOptionalPropertyTypes` happy.
      - Delegates:
        - [`enumerateChainCaptureSegments`](src/shared/engine/captureChainHelpers.ts:192) → aggregate `enumerateChainCaptureSegments`.
        - [`getChainCaptureContinuationInfo`](src/shared/engine/captureChainHelpers.ts:213) → aggregate `enumerateChainCaptureSegments`, re‑exposed as `{ hasFurtherCaptures, segments }`.
        - [`canCapture`](src/shared/engine/captureChainHelpers.ts:239) → shared [`validateCaptureSegmentOnBoard`](src/shared/engine/core.ts:243).
        - [`getValidCaptureTargets`](src/shared/engine/captureChainHelpers.ts:265) → aggregate `enumerateCaptureMoves` grouped by `captureTarget`.
        - [`processChainCapture`](src/shared/engine/captureChainHelpers.ts:312) → uses `canCapture` plus aggregate enumeration on the _unmutated_ state; documented as an approximate, analysis‑only helper.
  - Mutator shim:
...
- [`captureChainHelpers.ts`](src/shared/engine/captureChainHelpers.ts) is explicitly documented as:
...
- **Landing on opponent marker:** per canonical spec ([`Capture Chain Canonical Semantics`](docs/SHARED_ENGINE_CONSOLIDATION_PLAN.md:196) and [`RR-CANON-R101`](RULES_CANONICAL_SPEC.md:293)), capture enumeration and validation must treat “landing on an opponent marker” as illegal. This is enforced by:
  - Using [`validateCaptureSegmentOnBoard`](src/shared/engine/core.ts:243) for all segment legality checks.
  - Ensuring aggregate enumerators never surface such segments in their `Move` lists.
  - [`mutateCapture`](src/shared/engine/aggregates/CaptureAggregate.ts:632) still defensively removes an opponent marker found at the landing cell to preserve the “no stack + marker overlap” invariant for any pre‑existing states; this is treated as defensive clean‑up, not a legal move surface.
...
- **TS coverage:** targeted Jest runs over capture‑focused suites (including `captureLogic.shared`, `captureChainHelpers.shared`, `captureSequenceEnumeration`, `GameEngine.chainCapture`, and [`CaptureAggregate.chainCapture.shared`](tests/unit/CaptureAggregate.chainCapture.shared.test.ts:1)) now report:
  ...
  - [`captureChainHelpers.ts`](src/shared/engine/captureChainHelpers.ts:1) — ~92% statements/lines and >80% branches.
...
- [`sandboxCaptureSearch.findMaxCaptureChains()`](src/client/sandbox/sandboxCaptureSearch.ts:75)
  ...
  - Its header comment explicitly documents:
    ...
    - All canonical legality and continuation semantics must instead come from `CaptureAggregate` (or shims in [`captureLogic.ts`](src/shared/engine/captureLogic.ts:1) / [`captureChainHelpers.ts`](src/shared/engine/captureChainHelpers.ts:1)).
...
This section captures the **single source of truth** for overtaking capture-chains across all hosts. It is grounded in [`RR-CANON-R100–R103`](RULES_CANONICAL_SPEC.md:281) and the termination analysis in [`RULES_TERMINATION_ANALYSIS.md`](docs/supplementary/RULES_TERMINATION_ANALYSIS.md:19), and in the current shared TS implementation in [`CaptureAggregate.ts`](src/shared/engine/aggregates/CaptureAggregate.ts) and [`captureLogic.ts`](src/shared/engine/captureLogic.ts).
...
- Landing cell:
  ...
  - Contains no stack.
  - If it has a marker, that marker must belong to P.
  - Landing on an opponent marker is **illegal** and must be excluded by enumeration / validation.
...
3. **Global termination of the game**
   ...
   - In these cases, the game ends after the chain, but before the next player acts, per [`RR-CANON-R170–R173`](RULES_CANONICAL_SPEC.md:425) and the S-invariant analysis in [`RULES_TERMINATION_ANALYSIS.md`](docs/supplementary/RULES_TERMINATION_ANALYSIS.md:21).
...
- Cycles do **not** threaten termination because:
  - Every legal movement or capture strictly increases the progress metric `S = M + C + E`, and `S` is bounded above, as proven in [`RR-CANON-R191`](RULES_CANONICAL_SPEC.md:475) and elaborated in [`RULES_TERMINATION_ANALYSIS.md`](docs/supplementary/RULES_TERMINATION_ANALYSIS.md:21).
...
Python already defines [`ChainCaptureState`](ai-service/app/models.py:1); hosts should continue to own this and update it using the helpers below.
...
- [`update_chain_capture_state_py()`](ai-service/app/rules/core.py:246)
  - Python analogue of TS [`updateChainCaptureStateAfterCapture`](src/shared/engine/aggregates/CaptureAggregate.ts:800), updating the `chain_capture_state` field on `GameState` using the shared [`ChainCaptureState`](ai-service/app/models.py:1) dataclass.
...
- Strict invariants and soak tests documented in [`STRICT_INVARIANT_SOAKS.md`](docs/STRICT_INVARIANT_SOAKS.md:1).
...
- [ ] TS unit tests for line scenarios (`tests/unit/GameEngine.lines.scenarios.test.ts`, `tests/unit/ClientSandboxEngine.lines.test.ts`, `tests/unit/lineDecisionHelpers.shared.test.ts`) pass unchanged.
...
**Input Dependencies:** P1–P3, shared orchestrator design in [`RULES_ENGINE_CONSOLIDATION_DESIGN.md`](docs/drafts/RULES_ENGINE_CONSOLIDATION_DESIGN.md), existing [`turnLogic.ts`](src/shared/engine/turnLogic.ts) and [`orchestration/phaseStateMachine.ts`](src/shared/engine/orchestration/phaseStateMachine.ts)
...
- [ ] Backend uses a `ServerEngineAdapter` (as designed in [`RULES_ENGINE_CONSOLIDATION_DESIGN.md`](docs/drafts/RULES_ENGINE_CONSOLIDATION_DESIGN.md)) that delegates `processMove` to `processTurn` with proper decision resolution via existing interaction handlers.
...
- [ ] Deprecated backend and sandbox rules modules (listed in [`RULES_ENGINE_CONSOLIDATION_DESIGN.md`](docs/drafts/RULES_ENGINE_CONSOLIDATION_DESIGN.md)) are removed or reduced to trivial adapters: `src/server/game/rules/lineProcessing.ts`, `src/server/game/rules/territoryProcessing.ts`, [`sandboxLines.ts`](src/client/sandbox/sandboxLines.ts), [`sandboxLinesEngine.ts`](src/client/sandbox/sandboxLinesEngine.ts), [`sandboxTerritory.ts`](src/client/sandbox/sandboxTerritory.ts), [`sandboxTerritoryEngine.ts`](src/client/sandbox/sandboxTerritoryEngine.ts), [`sandboxCaptures.ts`](src/client/sandbox/sandboxCaptures.ts) as applicable.
- [ ] Documentation updated: `RULES_ENGINE_ARCHITECTURE.md`, [`CANONICAL_ENGINE_API.md`](docs/CANONICAL_ENGINE_API.md), and `CURRENT_RULES_STATE.md` reflect the new consolidated architecture.
