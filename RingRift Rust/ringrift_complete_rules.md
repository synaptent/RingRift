...
- Follow the unified movement landing rule (Section 8.2): you may land on **any valid space beyond the markers** along the line of travel, provided it is either **(a) empty** or **(b) occupied by a single marker of the moving stack's color**, and the total distance from start to landing is at least your stack's height.
...
### 10.3 Chain Overtaking
 ...
 • Must continue capturing as long as legal captures exist from the landing space of the prior Overtaking segment
...
#### Core Territory Concepts
 ...
 • **Connectivity Definition**: Territory connectivity uses the appropriate adjacency rule for the game version:
...
### 12.2 Disconnection Process
 ...
 • **Self-Elimination Prerequisite Check (Per Region):** Before you process a specific disconnected region, you must already have at least one ring or stack cap under your control that is located *outside that particular region*.
 ...
 • If the prerequisite **is** satisfied:
    ...
    - Finally, you *must* eliminate one of your remaining controlled rings or the entire cap of one of your remaining controlled stacks located outside the just-processed region.
...
• If the check passes, the disconnected region can be processed / collapsed by the player moving.
• If there are multiple disconnected regions that can be processed / collapsed by the player moving, the player can choose one of them at a time for processing / collapsing.
• When processing a selected disconnected region, the player collapses the region (and any single-color border markers) to their color, eliminates all internal rings (regardless of color), and *then* performs the mandatory self-elimination of one of their remaining rings/caps.
• On processing and collapsing a disconnected region, all rings within the region are Eliminated from play. For example, if rings controlled by three different players are present on the board, any region that is orthogonally disconnected from the rest of the board and contains only rings/stacks controlled by only one color or only two colors, is considered surrounded/disconnected, and all rings in all stacks of all colors within that region are Eliminated by the player who is moving and who caused the region to become disconnected.
