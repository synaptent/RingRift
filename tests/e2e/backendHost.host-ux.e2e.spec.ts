...
test.describe('backend host @host-ux', () => {
  ...
  test('supports keyboard navigation and keyboard-initiated ring placement', async ({ page }) => {
    ...
    console.log(`Keyboard nav: start=(${startX},${startY}) target=(${targetX},${targetY})`);
    ...
    const targetCell = boardView.locator(`button[data-x="${targetX}"][data-y="${targetY}"]`);
  ...
  test('exposes spectator view that mirrors game state and blocks move submission', async ({
    page,
    browser,
  }) => {
    ...
    try {
      ...
      await expect(spectatorPage.getByTestId('board-view')).toBeVisible({ timeout: 30_000 });
  ...
  test('keeps spectator HUD and evaluation panel after temporary network outage on /spectate/:gameId', async ({
    page,
    browser,
  }) => {
    // Create a backend game as an authenticated player.
    ...
    // Open a second context as a spectator.
    const spectatorContext = await browser.newContext();
    const spectatorPage = await spectatorContext.newPage();
    ...
    try {
      ...
      // Simulate a temporary network outage for the spectator context only.
      await spectatorContext.setOffline(true);
      // eslint-disable-next-line no-console
      console.log('Spectator context set offline for reconnect test');

      await spectatorContext.setOffline(false);
      // eslint-disable-next-line no-console
      console.log('Spectator context restored to online for reconnect test');
      ...
      await expect(evalPanel).toBeVisible({ timeout: 15_000 });
      await expect(spectatorGamePage.boardView).toBeVisible({ timeout: 30_000 });
    } finally {
      await spectatorContext.close();
    }
  });

  test('shows connection-loss banner and recovers after temporary network outage', async ({
    page,
    context,
  }) => {
    await registerAndLogin(page);
