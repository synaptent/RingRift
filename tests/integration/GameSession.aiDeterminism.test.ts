...
// Skip with orchestrator adapter - this test spies on GameSession.prototype.startTurnTimer
// which may not exist with the orchestrator adapter's different timer management.
const skipWithOrchestrator = process.env.ORCHESTRATOR_ADAPTER_ENABLED === 'true';

(skipWithOrchestrator ? describe.skip : describe)(
  'GameSession + AI determinism (per-game rngSeed)',
  () => {
    ...
    it('produces identical AI moves and post-move GameState across sessions', async () => {
      ...
      const lastMove1: Move | undefined =
        state1After.moveHistory[state1After.moveHistory.length - 1];
      const lastMove2: Move | undefined =
        state2After.moveHistory[state2After.moveHistory.length - 1];
