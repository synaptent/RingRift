...
| `npm run test:coverage:rules-critical`     | Rules-level TS suites plus shared-engine and contract-vector tests, with non-zero coverage thresholds over `src/shared/engine/**` excluding legacy files      | Layer 2 (canonical rules)         | Reviewer-facing coverage ratchet for the executable rules SSoT; runs inside `scripts/check_supported_path.sh`.                                                     |
| `npm run test:coverage:training-contracts` | Python encoder/model-versioning/minimal-loop/probe contract tests, measured with coverage.py and per-file non-zero ratchets                                   | Layer 2–3 (AI training contracts) | Reviewer-facing coverage ratchet for the supported Python training contract surface; runs inside `scripts/check_supported_path.sh`.                                |
