...
| `npm run test:ts-rules-engine`          | Rules‑level TS suites: `RefactoredEngine`, `*.rules.*`, `RulesMatrix.*`                                                                                       | Layer 2 (canonical rules)        | Primary semantic anchor for TS rules; pairs with contract vectors and FAQ/RulesMatrix scenarios (see also `docs/TEST_INFRASTRUCTURE.md` layer table).      |
