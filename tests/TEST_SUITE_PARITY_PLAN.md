...
| `tests/unit/RefactoredEngineParity.test.ts`                 | Rules-level + adapter parity  | TS shared engine ↔ backend/sandbox   | Verifies that backend/sandbox adapters behave like the refactored shared engine. Shared engine is the canonical **executable** implementation of the written rules spec.            | For any failure: fix adapters or add a more targeted rules-level test; do **not** diverge shared engine to preserve adapter quirks. |
...
| `tests/unit/TraceParity.seed17.firstDivergence.test.ts`           | Trace-level diagnostic                         | Backend ↔ sandbox                          | Same as seed 17 anchors.                                                                                                                                     | Historical; semantics now captured by `Seed17Move16And33Parity` / `Seed17Move52Parity` parity suites.                                                                                                                                                           |
...
| ------------------------------------------------------------------ | ------------------------------------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tests/parity/Backend_vs_Sandbox.eliminationTrace.test.ts`         | Trace-level parity                          | Backend GameEngine ↔ sandbox | Elimination and territory traces derived from shared-engine helpers and `RULES_SCENARIO_MATRIX.md`.                       | Ensure elimination and territory scenarios are covered in shared-engine rules tests and contract vectors.                                                                                       |
...
| `tests/parity/Backend_vs_Sandbox.seed5.*.test.ts`                  | Trace-level checkpoint/bisect parity        | Backend GameEngine ↔ sandbox | Seed-5 checkpoint, prefix, and bisect parity runs over full-game traces using shared-engine semantics as authority.       | Treat as smoke/regression nets; add dedicated rules-level tests when a seed reveals a true semantics issue.                                                                                     |
| `archive/tests/unit/TerritoryParity.GameEngine_vs_Sandbox.test.ts` | Trace-level parity (archived)               | Territory parity             | `territoryProcessing.shared.test.ts`, `territoryDecisionHelpers.shared.test.ts`, `BoardManager.territoryDisconnection.*`. | **Archived diagnostic harness.** Canonical territory semantics now live in shared decision-helper tests and RulesMatrix territory suites; keep this file for historical reference only.         |
...
- `tests/unit/GameEngine.victory.LPS.crossInteraction.test.ts`
- `tests/unit/GameEngine.lpsTracking.test.ts`
- `tests/unit/GameEngine.gameEndExplanation.shared.test.ts`
...
- `tests/unit/BoardManager.territoryDisconnection.square8.test.ts`
- `tests/unit/BoardManager.territoryDisconnection.hex.test.ts`
...
- `tests/unit/GameSession.aiChoiceCancellation.test.ts`
- `tests/unit/GameSession.sessionTerminateAICancellation.test.ts`
...
- `tests/unit/MetricsService.aiRequestTerminals.test.ts` – exercises AI/decision terminal metrics
  wiring (`ringrift_ai_turn_request_terminal_total`, `ringrift_ai_request_latency_ms`,
...
- **Fixture-driven rules parity**:
  - TS shared engine generates canonical fixtures (see §5 below), including:
    - Single-region mixed line+territory snapshots:
      - Exported via `tests/unit/ExportLineAndTerritorySnapshot.test.ts` with
        `npm run test:export-line-territory` (writes
        `line_territory_scenario_{square8,square19,hexagonal}.snapshot.json`
        under `ai-service/tests/parity/`).
    - Multi-region mixed line+territory snapshot:
      - Exported via `tests/unit/ExportLineAndTerritoryMultiRegionSnapshot.test.ts` with
        `npm run test:export-line-territory-multi-region` (writes
        `line_territory_multi_region_square8.snapshot.json` under `ai-service/tests/parity/`).
